  ...
  var chromeEmu = {
    ...
    xmlHttpRequest: function (details, callback, onreadychange, onerr, done) {
      var cb = function (req) {
        if (EMV)
          console.log(
            "emu: req (" + id + " -" + details.url + " returned: " + req,
          );
        ...
        if (req.onReadyStateChange && onreadychange)
          onreadychange(req.response);
      ...
      var run = function () {
        tmCE.xmlHttpRequest(chromeEmu.key, d, id);
      };
    ...
    runUpdateListeners: function (tabId, details, tab) {
      if (EMV)
        console.log("emu: runUpdateListeners " + tabId + " URL: " + tab.url);
      for (var k = 0; k < chromeEmu.tabs.updateListeners.length; k++) {
        ...
        try {
          ...
          console.log(
            "emu: Error (runUpdateListeners):" + e + "\n" + l.toString(),
          );
        }
      }
    },
    ...
    runRequestHandlers: function (request, sender, responseId, responseTab) {
      ...
      for (var k = 0; k < chromeEmu.extension.requestHandlers.length; k++) {
        ...
        try {
          ...
          console.log(
            "emu: Error (runRequestHandlers):" + e + "\n" + l.toString(),
          );
        }
      }
    },

    runContentRequestHandlers: function (
      request,
      sender,
      responseId,
      responseTab,
      ...
      for (var k = 0; k < chromeEmu.extension.requestHandlers.length; k++) {
        ...
        try {
          ...
          console.log(
            "emu: Error (runContentRequestHandlers):" + e + "\n" + l.toString(),
          );
        }
      }
    },
        ...
        if (EMV)
          console.log(
            "emu: runRequest " +
              responseId +
              " -> " +
              Converter.decode(request),
          );
        var r = JSON.parse(Converter.decode(args));
        chromeEmu.runRequestHandlers(
          r.request,
          r.sender,
          responseId,
          responseTab,
    ...
    runContentRequest: function (responseId, args, responseTab) {
      try {
        if (EMV)
          console.log(
            "runContentRequest " + responseId + " -> " + Converter.decode(args),
          );
        var r = JSON.parse(Converter.decode(args));
        chromeEmu.runContentRequestHandlers(
          r.request,
          r.sender,
          responseId,
          responseTab,
        ...
        console.log(
          "emu: Json parse error (runContentRequest):" + e + "\n" + args,
        );
      }
    },
        ...
        for (var k in chromeEmu.responses) {
          if (k == responseId) {
            ...
            try {
              ...
              console.log(
                "emu: Json parse error (runResponse):" + e + "\n" + args,
              );
            }
            p = "";
        ...
        for (var k in chromeEmu.responses) {
          if (k == responseId) {
            ...
            try {
              ...
              console.log(
                "emu: Json parse error (runConnectResponse):" + e + "\n" + args,
              );
            }
            p = "";
    ...
    extension: {
      ...
      sendMessage: function (obj, response) {
        ...
        if (EMV)
          console.log(
            "emu: extension.sendMessage " + id + " # " + JSON.stringify(obj),
          );
