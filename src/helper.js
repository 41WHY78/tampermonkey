...
(function () {
  ...
  const getStringBetweenTags = function (source, tag1, tag2) {
    ...
    const e = source.substr(b + tag1.length).search(escapeForRegExp(tag2));
  ...
  const escapeForRegExpURL = function (str, more) {
  ...
  const getRegExpFromUrl = function (url, cfg, safe, match) {
    let u;
    if ((cfg.values.tryToFixUrl || safe) && url == urlAllInvalid) {
      u = urlAllHttp;
    } else if (
      (cfg.values.safeUrls || safe) &&
      url != urlAllHttp &&
      url != urlAllHttps &&
      url.search(escapeForRegExpURL(urlSecurityIssue)) != -1
    ) {
  ...
  const isLocalImage = function (url) {
    const bg = "background.js";
    let u = chrome.extension.getURL(bg);
  ...
  const getUrlArgs = function () {
    const c = {};
    const p = window.location.search.replace(/^\?/, "");
    const args = p.split("&");
    let pair;
  ...
  const doConfirm = function (msg, cb) {
    const t = function () {
      const r = confirm(msg);
      cb(r);
    };
