  ...
  var createTabView = function (_prefix, parent, style) {
    ...
    var setHtmlVisible = function (elem, vis, move) {
      if (vis) {
        elem.setAttribute(
          "style",
          move ? Helper.staticVars.visible_move : Helper.staticVars.visible,
        );
      } else {
        elem.setAttribute(
          "style",
          move ? Helper.staticVars.invisible_move : Helper.staticVars.invisible,
        );
      }
    ...
    var selectTab = function (tab) {
      ...
      if (tvCache[prefix].g_selectedId) {
        setContentVisible(
          tvCache[prefix].g_entries[tvCache[prefix].g_selectedId],
          false,
        );
      }
    ...
    var hideTab = function (tab) {
      ...
      if (sel) {
        ...
        if (f) {
          ...
          tvCache[prefix].g_selectedId = null;
          console.log(
            "tv: WARN: selected tab set, but entry collection empty!",
          );
        }
      }
    };
      ...
      tv = {
        ...
        insertTab: function (before, id, head, cont, selectCb, closeCb) {
          ...
          var oc = function (e) {
            ...
            if (
              e.target.className != "" &&
              e.target.className == style.tv_tab_close
            )
              return;
            entry.select();
          };
          ...
          tvCache[prefix].g_entries[tid] = {
            entry: entry,
            tab: tab,
            content: cont,
            closable: closeCb != null,
          };
