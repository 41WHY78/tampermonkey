...
Registry.registerRaw("environment.js", function () {
  ...
  var TM_isPartOfDOM = function (elem, cnt) {
    ...
    return (
      cnt &&
      elem &&
      (elem == document || TM_isPartOfDOM(elem.parentNode, cnt - 1))
    );
  };
    ...
    try {
      ...
      for (var p in addProps.elements) {
        ...
        if (t.name) {
          ...
          if (D)
            console.log(
              "env: WARN: unexpected item in props elem: " + JSON.stringify(t),
            );
        }
      }
      ...
      var wrap = function (source) {
        ...
        s += "} catch (e) {\n";
        s +=
          "    console.log(\"ERROR: Execution of script '" +
          script.name +
          "' failed! \" + e.message); \n";
        s +=
      ...
      var fail = function () {
        if (D) {
          chromeEmu.extension.sendMessage(
            {
  ...
  var domNodeInsertedListener = function (e) {
    if (!TMwin.domNodeInserted && (V || EV || D))
      console.log("env: first DOMNodeInserted Event!");
    TMwin.loadHappened = true;
    TMwin.domNodeInserted = true;
    if (nodeInserts) {
      nodeInserts.events.push({
        event: e,
        domContentLoaded: TMwin.domContentLoaded,
      });
    }
  };
  ...
  var runAllLoadListeners = function () {
    if (!TMwin.domContentLoaded) {
      if (V || EV || D)
        console.log("env: Content not loaded, schedule loadListeners run!");
      return -1;
    }
    ...
    while (loadListeners.length > 0) {
      var run = function () {
        ...
        try {
          ...
          console.log(
            "ERROR: Execution (loadListener) of script env " +
              fn.name +
              " failed!" +
              e.message,
          );
      ...
      var wrap = function () {
        ...
        if (!k.__addEventListener) {
          ...
          k.removeEventListener = function (event, fn, arg1) {
            if (event == eDOMNODEINSERTED) {
              if (nodeInserts && nodeInserts.running == fn) {
                if (EV)
                  console.log(
                    "env: detected removeEventListener while refireAllNodeInserts",
                  );
          ...
          k.addEventListener = function (event, fn, arg1) {
            ...
            if (
              event == eLOAD ||
              event == eDOMCONTENTLOADED ||
              event == eDOMNODEINSERTED
            ) {
              var sid = null;
              var that = this;
              try {
              ...
              if (sid) {
                ...
                if (event == eLOAD) {
                  ...
                  if (nodeInserts && !nodeInserts.done[sid]) {
                    ...
                    run = function () {
                      var onlyEventsAfterDomLoaded =
                        sid.run_at != "document-start" &&
                        sid.run_at != "document-body";

                      nodeInserts.running = fn;
                      refireAllNodeInserts(
                        fn,
                        that,
                        sid,
                        onlyEventsAfterDomLoaded,
                      );
      ...
      window[k].prototype.evaluate = function (query, elem, arg1, arg2, arg3) {
        ...
        if (typeof XPathResult != "undefined") {
          ...
          try {
            ...
            if (V)
              console.log("env: Error: document.evaluate " + JSON.stringify(e));
          }
          ...
          if (V)
            console.log(
              "env: XPathResult == undefined, but selectNodes via " + xpathExpr,
            );
  ...
  var TM_registerMenuCommand = function (name, fn) {
    ...
    var onUnload = function () {
      if (V)
        console.log("env: unRegisterMenuCMD due to unload " + fn.toString());
      chromeEmu.extension.sendMessage(
        { method: "unRegisterMenuCmd", name: name, id: menuId },
        function (response) {},
      );
    };
  ...
  var TM_openInTab = function (url, options) {
    ...
    var close = function () {
      if (tabId === null) {
        ...
        chromeEmu.extension.sendMessage(
          { method: "closeTab", tabId: tabId, id: TM_context_id },
          resp,
        );
        tabId = undefined;
      } else {
    ...
    chromeEmu.extension.sendMessage(
      { method: "openInTab", url: url, id: TM_context_id, options: options },
      resp,
    );
    return { close: close };
  };
    ...
    if (/* TMwin.use.safeContext */ xmlhttp2) {
      ...
      var plist = function (response) {
        try {
          if (response.success) {
            if (details["onload"]) {
              if (response.data.responseXML)
                response.data.responseXML = unescape(response.data.responseXML);
      ...
      var omsg = function (response) {
        console.log("env: onDisconnect! :)");
      };
  ...
  var TM_getTab = function (cb) {
    chromeEmu.extension.sendMessage(
      { method: "getTab", id: TM_context_id },
      function (response) {
        if (cb) {
          cb(response.data);
        }
  ...
  var TM_saveTab = function (tab) {
    chromeEmu.extension.sendMessage(
      { method: "saveTab", id: TM_context_id, tab: tab },
      function (response) {},
    );
  };
  ...
  var TM_getTabs = function (cb) {
    chromeEmu.extension.sendMessage(
      { method: "getTabs", id: TM_context_id },
      function (response) {
        if (cb) {
          cb(response.data);
        }
  ...
  var TM_installScript = function (url, cb) {
    chromeEmu.extension.sendMessage(
      { method: "scriptClick", url: url, id: TM_context_id },
      function (response) {
        if (cb) cb(response);
      },
    );
  ...
  var HTM_runMyScript = function (HTM_request) {
    ...
    var HTM_getInfo = function () {
      var excl = { observers: 1, id: 1, enabled: 1, hash: 1, fileURL: 1 };
      var o = { script: {} };
      ...
      o.script["run-at"] =
        HTM_script["options"].override.run_at || HTM_script["options"].run_at;
      ...
      o["scriptWillUpdate"] = !!(
        HTM_script["options"].fileURL && HTM_script["options"].fileURL != ""
      );
    ...
    var HTM_initStoragePort = function () {
        ...
        if (response.storage) {
          for (var k in response.storage.data) {
            ...
            var run = function () {
              ...
              if (V)
                console.log(
                  "env: storageListener - config key " +
                    key +
                    ": " +
                    oldval +
                    " -> " +
      ...
      storagePort = chromeEmu.extension.connect(
        "storageListener_" + TM_context_id,
      );
      storagePort.onMessage.addListener(storageListener);
      var omsg = function (response) {
        console.log("env: storageListener onDisconnect! :)");
      };
      if (V) storagePort.onDisconnect.addListener(omsg);

      storagePort.postMessage({
    ...
    var HTM_saveStorageKey = function (key) {
      ...
      if (V)
        console.log(
          "env: saveStorageKey - config key " +
            key +
            ": " +
            TM_context_storage.data[key],
        );
    };
    ...
    var TM_notifyValueChangeListeners = function (
      name,
      oldVal,
      newVal,
      remote,
    ...
    var TM_deleteValue = function (name) {
      ...
      TM_notifyValueChangeListeners(
        name,
        old,
        TM_context_storage.data[name],
        false,
      );
    };
    ...
    var TM_notification = function (msg, title, image, cb, delay) {
      ...
      if (image == undefined) {
        image = HTM_request.script.icon
          ? HTM_request.script.icon
          : HTM_request.script.icon64;
      }
    ...
    if (TMwin.props[HTM_script["namespace"]] == undefined) {
      TMwin.props[HTM_script["namespace"]] = {
        scriptid: HTM_request.script.id,
        context: function () {},
        elements: [],
      };
    ...
    if (!TMwin.use.safeContext) {
      ...
      for (var k in unsafe) {
        ...
        var run = function () {
          ...
          TMwin.props[HTM_script["namespace"]].elements.push({
            name: "unsafe" + cc,
            value: unsafe[k],
          });
        };
    ...
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_registerMenuCommand",
      value: TM_registerMenuCommand,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_openInTab",
      value: TM_openInTab,
    ...
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_addValueChangeListener",
      value: TM_addValueChangeListener,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_removeValueChangeListener",
      value: TM_removeValueChangeListener,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_xmlhttpRequest",
      value: TM_xmlhttpRequest,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_setClipboard",
      value: TM_setClipboard,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_getTab",
      value: TM_getTab,
    ...
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_installScript",
      value: TM_installScript,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_runNative",
      value: TM_runNative,
    ...
    TMwin.props[HTM_script["namespace"]].elements.push({
      ...
      value: TM_notification,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_getResourceText",
      value: TM_getResourceText,
      scriptid: HTM_script.id,
    });
    TMwin.props[HTM_script["namespace"]].elements.push({
      name: "TM_getResourceURL",
      value: TM_getResourceURL,
      scriptid: HTM_script.id,
    });
      ...
      if (!Object.prototype.toSource) {
        Object.defineProperties(Object.prototype, {
          toSource: {
            value: function () {
              return (
                "JSON.parse(unescape('" + escape(JSON.stringify(this)) + "'));"
              );
            },
            enumerable: false,
            writable: true,
  ...
  chromeEmu.extension.onMessage.addListener(
    function (request, sender, sendResponse) {
      if (V || EV)
        console.log(
          "env: request.method " + request.method + " id: " + request.id,
        );
      ...
      if (request.method == "executeScript") {
        ...
        if (request.script.options.run_at == "document-start") {
          if (D)
            console.log(
              "env: run '" + request.script.name + "' ASAP -> document-start",
            );
          TM_runASAP(r, request.script.id);
        } else if (request.script.options.run_at == "document-body") {
          if (D)
            console.log(
              "env: schedule '" + request.script.name + "' for document-body",
            );
          TM_runBody(r, request.script.id);
        } else {
          if (D)
            console.log(
              "env: schedule '" + request.script.name + "' for document-end",
            );
        ...
        sendResponse({});
        window.setTimeout(function () {
          if (V || EV) console.log("env: disable nodeInserts magic!");
          nodeInserts = null;
        }, 2000);
      } else if (topframe) {
