...
(function () {
  ...
  const compaMo = Registry.get("compat");

  const getScriptId = function (name) {
    let id = null;
    ...
    if (ch) {
      ...
      id = Converter.Base64.encode(name)
        .match(/[a-zA-Z0-9]/g)
        .join("");
    }
    return id;
  ...
  var scriptParser = {
    ...
    processMetaHeader: function (header) {
      ...
      const tags = ["uso:hash", "version", "name"];
      ...
      for (const t in tags) {
        meta[tags[t]] = Helper.getStringBetweenTags(
          header,
          "@" + tags[t],
          "\n",
        ).trim();
      ...
      if (V || UV) {
        console.log("parser: processMetaHeader -> " + JSON.stringify(meta));
      }
    ...
    processHeader: function (header) {
      ...
      const icon_tags = ["iconURL", "defaulticon", "icon"];
      const icon64_tags = ["icon64URL", "icon64"];
      const homepage_tags = ["homepageURL", "homepage", "website", "source"];
      ...
      for (i in lines) {
        ...
        for (var t in tags) {
          const r = new RegExp("^@" + tags[t] + "[\\t\\s]");
          if (l.search(r) != -1) {
            script[tags[t]] = Helper.getStringBetweenTags(
              l,
              "@" + tags[t],
            ).trim();
            continue;
      ...
      if (script.name) {
        ...
        if (D) {
          console.log("parser: script " + script.name + " got id " + script.id);
        }
      }
    ...
    getHeader: function (src) {
      const header = Helper.getStringBetweenTags(src, headerStart, headerStop);
      ...
      const b1 = "<html>";
      const b2 = "<body>";
      const p0 = src.search(headerStart);
      const p1 = src.search(b1);
      const p2 = src.search(b2);

      if ((p1 > 0 && p1 < p0) || (p2 > 0 && p2 < p0)) {
    ...
    createScriptFromSrc: function (src) {
      ...
      const header = scriptParser.getHeader(src);
      if (!header) return {};
      const script = scriptParser.processHeader(header);
      ...
      if (!script.options.awareOfChrome) {
        ...
        script.options.compat_arrayleft =
