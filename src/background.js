...
(function () {
  ...
  var TM_instanceID =
    new Date().getTime() + Math.floor(Math.random() * 061283 + 1);
  var TM_tabs = {};
  ...
  var convertData = function (convertCB) {
        ...
        var wrap = function () {
          ...
          for (var kk in d.options) {
            ...
            if (r.script.options[kk] === undefined) {
              console.log(
                "set option " + kk + " to " + JSON.stringify(d.options[kk]),
              );
            ...
            if (r.script.options.override[e] === undefined) {
              console.log(
                "set option.override." +
                  e +
                  " to " +
                  JSON.stringify(d.options.override[e]),
              );
    ...
    checks = [
      ...
      {
        cond:
          isNewVersion && _use_localdb && versionCmp("1.2", version) == eNEWER,
        fn: function (cb) {
          ...
          for (var i = 0; i < localStorage.length; i++) {
            ...
            if (value) {
              if (V)
                console.log("Copy from localStorage: " + name + " -> " + value);
      ...
      },
      {
        ...
        fn: function (cb) {
          ...
          for (var k in names) {
            ...
            r.script.options.compatopts_for_requires =
              r.script.options.compat_for_requires;
            storeScript(r.script.name, r.script, false);
          }
          window.setTimeout(cb, _setTimeout);
        },
  ...
  var getFromRequireCache = function (url) {
    ...
    if (V || CV)
      console.log("cache: " + (ret ? "found" : "missed") + " '" + url + "'");
    return ret;
  };
  ...
  var ctxRegistry = {
    ...
    addUrl: function (tabId, frameId, url, ua) {
      ...
      if (ctxRegistry.n[tabId].urls[url] == undefined)
        ctxRegistry.n[tabId].urls[url] = 0;
      ...
      for (var k in ua) {
        ...
        if (!ctxRegistry.n[tabId].user_agent)
          ctxRegistry.n[tabId].user_agent = {};
    ...
    setCache: function (tabId, frameId, url, runInfo) {
      if (V || UV || EV)
        console.log("ctxReg: setCache to ctxRegistry[" + tabId + "] -> " + url);
      ctxRegistry.assert(tabId, false);
    ...
    clearCache: function (tabId, frameId) {
      if (V || UV || EV)
        console.log("ctxReg: clearCache to ctxRegistry[" + tabId + "]");
      if (ctxRegistry.has(tabId)) {
    ...
    removeUrl: function (tabId, frameId, url) {
      ...
      if (--ctxRegistry.n[tabId].urls[url] == 0) {
        if (V || UV || EV)
          console.log(
            "ctxReg: remove from ctxRegistry[" + tabId + "] -> " + url,
          );
  ...
  var Tab = {
    getScriptRunInfo: function (url, frameId) {
      ...
      for (var k = 0; k < scripts.length; k++) {
        ...
        if (V)
          console.log(
            "check " + script.name + " for enabled:" + script.enabled,
          );
    ...
    runScripts: function (nfo, runInfo, allrun_cb) {
      ...
      if (!runInfo && ctxRegistry.has(nfo.tabId)) {
  ...
  var ScriptDetector = {
    isScriptUrl: function (url_string) {
      ...
      var p =
        url.search(/\.user\.(js\#|js\?|js$)/) != -1 ||
        url.search(/\.tamper\.(js\#|js\?|js$)/) != -1;
      if (!p) return p;

      var n =
        url.search(/^htt[ps]{1,2}:\/\/code\.google\.com/) !=
          -1 /* google code raw files are deliverd from googlecode.com */ ||
        (url.search(/^htt[ps]{1,2}:\/\/github\.com/) != -1 &&
          url.search(
            /^htt[ps]{1,2}:\/\/github\.com\/[a-zA-Z0-9%-]\/[a-zA-Z0-9%-]\/raw\//,
          ) == -1); /* install userscirpt only from /raw/ urls */
  ...
  var localFile = {
    ...
    listener: function (event, d) {
      ...
      try {
        ...
        console.log(
          "localFile: ERR: getSource processing of " + d + " failed!",
        );
      }
    },
    initialize: function () {
      ...
      var wrap = function () {
        ...
        var notfound = function () {
          ...
          if (localFile.callbacks[cbi]) {
            localFile.listener(
              null,
              JSON.stringify({ id: cbi, content: null }),
            );
          }
          cbi = null;
        };
  ...
  var TM_fire = {
    ...
    isReady: function () {
      return (
        TM_fire.status.initialized &&
        !TM_fire.status.update &&
        !TM_fire.status.download
      );
    },
    ...
    checkUpdate: function (force_check, force_update, check_cb, up_cb) {
      ...
      if (
        !force &&
        (Config.values.fire_updatePeriod == 0 || !Config.values.fire_enabled)
      )
        return;
      var last = getUpdateCheckCfg();
      ...
      if (
        force ||
        new Date().getTime() - last.fire.last > Config.values.fire_updatePeriod
      ) {
        var db = 0;
          ...
          if (req.readyState == 4) {
            if ((req.status = 200)) {
              try {
                ...
                console.log(
                  "bg: fire: unable to parse DB version response! " +
                    req.responseText,
                );
              }
              console.log(
                "bg: fire: local DB version: " +
                  last.fire.db_version +
                  " remote: " +
                  db,
              );
    ...
    update: function (cb) {
      ...
      var error = function (msg) {
        ...
        notify.show(
          "TamperFire",
          I18N.getMessage("TamperFire_update_failed___"),
      ...
      var progress = function (req) {
        setTimer();
        if (req.progress) {
          TM_fire.status.progress = {
            n: req.progress.total,
            of: req.progress.totalSize * 6,
          };
      ...
      notify.show(
        "TamperFire",
        I18N.getMessage("TamperFire_update_started"),
        chrome.extension.getURL("images/icon128.png"),
      );
    ...
    clean: function (cb) {
      ...
      var do5 = function () {
        TM_fire.fireDB.db.transaction(function (tx) {
          tx.executeSql("DROP TABLE scripts", [], done, done);
        });
      };

      var do4 = function () {
        TM_fire.fireDB.db.transaction(function (tx) {
          tx.executeSql("DROP TABLE excludes", [], do5, do5);
        });
      };

      var do3 = function () {
        TM_fire.fireDB.db.transaction(function (tx) {
          tx.executeSql("DROP TABLE includes", [], do4, do4);
        });
      };

      var do2 = function () {
        TM_fire.fireDB.db.transaction(function (tx) {
          tx.executeSql("DROP TABLE scriptexcludes", [], do3, do3);
        });
      };

      var do1 = function () {
        TM_fire.fireDB.db.transaction(function (tx) {
          tx.executeSql("DROP TABLE scriptincludes", [], do2, do2);
        });
      };
      ...
      TM_fire.fireDB = {
          ...
          TM_fire.fireDB.db.transaction(function (tx) {
            tx.executeSql(
              "CREATE TABLE IF NOT EXISTS " +
                "scripts(sid INTEGER PRIMARY KEY ASC, value TEXT)",
              [],
              aftercreate,
              err,
            );
        ...
        createIncludesTable: function (aftercreate) {
          ...
          TM_fire.fireDB.db.transaction(function (tx) {
            tx.executeSql(
              "CREATE TABLE IF NOT EXISTS " +
                "includes(iid INTEGER PRIMARY KEY ASC, generic BOOLEAN, regex TEXT)",
              [],
              aftercreate,
              err,
            );
        ...
        createExcludesTable: function (aftercreate) {
          ...
          TM_fire.fireDB.db.transaction(function (tx) {
            tx.executeSql(
              "CREATE TABLE IF NOT EXISTS " +
                "excludes(eid INTEGER PRIMARY KEY ASC, regex TEXT)",
              [],
              aftercreate,
              err,
            );
    ...
    insertValuesFromJSON: function (json, cb) {
      ...
      notify.show(
        "TamperFire",
        I18N.getMessage("TamperFire_import_started"),
        chrome.extension.getURL("images/icon128.png"),
      );
      ...
      var done = function () {
        var aDone = function () {
          ...
          notify.show(
            "TamperFire",
            I18N.getMessage("TamperFire_is_up_to_date"),
            chrome.extension.getURL("images/icon128.png"),
          );
        };
        var siDone = function () {
    ...
    getValues: function (table, keyname, keyvalues, cb) {
      ...
      var run = function (t) {
        ...
        tx.executeSql(
          "SELECT * FROM " + table + " WHERE " + t.join(" OR "),
          search,
          callback,
          TM_fire.fireDB.onError,
        );
    ...
    url: {
      getCount: function (url, cb) {
        ...
        inc_url +=
          "    (SELECT sid FROM scriptincludes WHERE iid IN (SELECT iid FROM includes WHERE generic=0 AND ? REGEXP regex)) ";
        inc_url += "AND NOT sid IN ";
        inc_url +=
          "    (SELECT sid FROM scriptexcludes WHERE eid IN (SELECT eid FROM excludes WHERE ? REGEXP regex)) ";

        TM_fire.fireDB.db.transaction(function (tx) {
          tx.executeSql(inc_url, [url, url], callback, TM_fire.fireDB.onError);
        });
      },
      ...
      getItems: function (url, cb) {
        ...
        if (u == 0) {
          // 1600
          inc_url +=
            "SELECT DISTINCT t1.* FROM scripts T1 JOIN scriptincludes T2 ON T1.sid=T2.sid WHERE T2.iid IN ";
          inc_url +=
            "    (SELECT iid FROM includes WHERE generic=0 AND ? REGEXP regex) ";
          inc_url += "AND NOT T1.sid IN ";
          inc_url +=
            "    (SELECT T4.sid FROM excludes T3 JOIN scriptexcludes T4 ON T3.eid=T4.eid WHERE T3.eid IN (SELECT eid FROM excludes WHERE ? REGEXP regex))";
        } else if (u == 1) {
          // 1134
          ...
          // 2700
          inc_url +=
            "SELECT DISTINCT t1.* FROM scripts T1 JOIN scriptincludes T2 ON T1.sid=T2.sid WHERE EXISTS";
          inc_url +=
            "    (SELECT iid FROM includes I1 WHERE T2.iid=I1.iid AND generic=0 AND ? REGEXP regex) ";
          inc_url += "AND NOT T1.sid IN ";
          inc_url +=
            "    (SELECT T4.sid FROM excludes T3 JOIN scriptexcludes T4 ON T3.eid=T4.eid WHERE T3.eid IN (SELECT eid FROM excludes WHERE ? REGEXP regex))";
        } else if (u == 3) {
          // 2800
          inc_url +=
            "SELECT DISTINCT t1.* FROM scripts T1 JOIN scriptincludes T2 ON T1.sid=T2.sid JOIN includes I1 ON I1.iid=T2.iid WHERE I1.generic=0 AND ? REGEXP I1.regex ";
          inc_url += "AND NOT T1.sid IN ";
          inc_url +=
            "    (SELECT T4.sid FROM excludes T3 JOIN scriptexcludes T4 ON T3.eid=T4.eid WHERE T3.eid IN (SELECT eid FROM excludes WHERE ? REGEXP regex))";
        }
        var inc_all =
          "SELECT DISTINCT t1.value, t1.sid FROM scripts T1 JOIN scriptincludes T2 ON T1.sid=T2.sid WHERE T2.iid IN (SELECT iid FROM includes WHERE generic=0)";
        ...
        var done = function (tx, vars) {
          t2 = new Date().getTime();
          if (D)
            console.log("bg: TF db access: " + u + " -> " + (t2 - t1) + "ms");
          if (vars.rows && vars.rows.length) {
        ...
        var fill = function (tx, vars) {
          ...
          if (vars) {
            for (var i = 0; i < vars.rows.length; i++) {
              ...
              TM_storage.cacheDB[vars.rows.item(i).name] =
                vars.rows.item(i).value;
            }
          }
        ...
        TM_storage.localDB = {
          ...
          createTable: function (aftercreate) {
            TM_storage.localDB.db.transaction(function (tx) {
              tx.executeSql(
                "CREATE TABLE IF NOT EXISTS " +
                  "config(ID INTEGER PRIMARY KEY ASC, name TEXT, value TEXT)",
                [],
                aftercreate,
                TM_storage.localDB.onError,
              );
  ...
  var SyncClient = {
    ...
    createTeslaData: function (cb) {
      ...
      for (var k = 0; k < local.length; k++) {
        if (local[k].url) {
          var s =
            local[k].name.replace(/\|/g, "!") +
            "|" +
            "{}" +
            "|" +
            local[k].url.replace(/\|/g, "%7C");
          ret.push(s);
        }
      }
      if (cb) cb(ret);
    ...
    enable: function (callback) {
      if (SyncClient.enabled) {
        ...
        SyncClient.enabled = SyncInfo.init(
    ...
    addSyncDoneListener: function (cb) {
      ...
      if (V)
        console.log(
          "sync: addSyncDoneListener() -> " +
            SyncClient.syncDoneListener.length,
        );
    },
    runAllSyncDoneListeners: function (success) {
      if (V)
        console.log(
          "sync: runAllSyncDoneListeners() -> " +
            SyncClient.syncDoneListener.length,
        );
        ...
        if (Config.values.sync_type == SyncInfo.types.eCHROMESYNC) {
          var got = function (has, asked) {
            if (has) {
              ...
              console.log(
                "sync: storage permission is needed in order to use Google Sync!",
              );
              abort();
            }
          };
    ...
    schedulePeriodicalCheck: function () {
      ...
      if (D)
        console.log(
          "sync: schedule sync for periodical run every " + t + " ms",
        );
      ...
      var localByUrl = function (u) {
        if (u) {
          ...
          for (var k = 0; k < local.length; k++) {
            ...
            if (local[k].furl == u || local[k].durl == u) {
              return local[k];
            }
          }
        }
      ...
      var impo = function () {
        ...
        for (var u = 0; u < remote.length; u++) {
          ...
          if (locally && o.options.removed) {
            change = true;
            if (D)
              console.log("sync: remove local script " + (o.name || o.url));
    ...
    remoteChangeCb: function (name, script) {
      if (
        !SyncClient.enabled ||
        Config.values.sync_type != SyncInfo.types.eCHROMESYNC
      )
        return;
  ...
  var ConfigObject = function (initCallback) {
    ...
    var setValue = function (name, value) {
      if (oobj.changeListeners[name] && _internal[name] != value) {
    ...
    var afterload = function () {
      ...
      if (oobj.values.notification_showTMUpdate && upNotification) {
        var args =
          ...
          "&" +
          "ext=" +
          chrome.extension.getID().substr(0, 4);
        var url = "http://tampermonkey.net/changelog.php?" + args;

        notify.showUpdate(
          I18N.getMessage(
            "Updated_to__0version0",
            upNotification || chrome.extension.getVersion(),
          ),
          null,
          chrome.extension.getURL("images/icon128.png"),
          {
            text: I18N.getMessage("Click_here_to_see_the_recent_changes"),
            src: url,
          },
        );
      }
  ...
  var xmlhttpRequestInternal = function (
    details,
    callback,
    onreadychange,
    onerr,
  ...
  var runtimeInit = function () {
    ...
    this.getNextResource = function (script, cb) {
      var storeResource = function (req, res) {
        ...
        for (var k in rh) {
          ...
          if (
            h.trim().toLowerCase() == "content-type" &&
          ...
          if (req.status == 200 || req.status == 0) {
            ...
            if (!image) {
              ...
              res.resURL =
                ...
                image +
                ";base64," +
                Converter.Base64.encode(req.responseText);
            }
            cb(script);
          } else {
            if (D || V)
              console.log(
                "getRes: Failed to load: '" +
                  res.url +
                  "' " +
                  req.status +
                  " " +
        ...
        if (!r.loaded) {
          ...
          if (t) {
            ...
            if (r.url.search("^file://") == 0) {
              var c = function (s) {
                storeResource(
                  { readyState: 4, status: s ? 0 : 404, responseText: s },
                  r,
                );
              };
    ...
    this.isBlacklisted = function (url) {
      ...
      var check = function (v) {
        ...
        if (D && b)
          console.log('bg: require blacklist entry "' + v + '" matched');
    ...
    this.getRequires = function (script, cb) {
      ...
      for (var k in script.requires) {
        ...
        if (!r.loaded && r.url) {
          ...
          if (t) {
            fillRequire(
              { readyState: 4, status: 200, responseText: t.content },
              r,
            );
    ...
    this.contentLoad = function (info, main, cb) {
      if (
        oobj.getNextResource(main, function (script) {
          oobj.contentLoad(info, script, cb);
        })
      ) {
        return;
      }
      ...
      oobj.info = info;
      if (typeof TM_tabs[info.tabId] == "undefined")
        TM_tabs[info.tabId] = { storage: {} };
      ...
      var req_cb = function () {
        ...
        console.log(
          I18N.getMessage("run_script_0url0___0name0", [info.url, main.name]),
        );
    ...
    this.injectScript = function (scripts, cb) {
      ...
      for (var i = 0; (script = scripts[i]); i++) {
        ...
        script.requires.forEach(function (req) {
          ...
          contents = compaMo.mkCompat(
            contents,
            script.options.compatopts_for_requires ? script : null,
          );
  ...
  var ts_ify = function (u) {
    if (u)
      u += (u.search("\\?") == -1 ? "?" : "&") + "ts=" + new Date().getTime();
    return u;
  };
  ...
  var determineSourceURL = function (o, add_ts) {
    ...
    if (o.downloadURL && o.downloadURL.search("^file://" == -1))
      f = o.downloadURL;
  ...
  var mergeCludes = function (script) {
    var n,
      cludes = script.options.override;

    //clone the original cludes as a starting point
    script.includes =
      cludes.merge_includes && cludes.orig_includes
        ? cludes.orig_includes.slice()
        : [];
    script.excludes =
      cludes.merge_excludes && cludes.orig_excludes
        ? cludes.orig_excludes.slice()
        : [];
    script.matches =
      cludes.merge_matches && cludes.orig_matches
        ? cludes.orig_matches.slice()
        : [];
  ...
  var notifyOptionsTab = function () {
    ...
    var done = function (allitems) {
      chrome.extension.sendMessage(
        { method: "updateOptions", items: allitems },
        function (response) {},
      );
    };
  ...
  var addNewUserScript = function (o) {
    ...
    if (!o.clean && oldscript && oldscript.system && !o.defaultscript)
      return false;
    ...
    if (script.options.compat_uW_gmonkey) {
      chrome.tabs.sendMessage(
        ...
        {
          method: "showMsg",
          msg: I18N.getMessage("This_script_uses_uW_gm_api_"),
        },
        function (response) {},
      );
    ...
    script.position = oldscript
      ? oldscript.position
      : determineLastScriptPosition() + 1;
    ...
    if (script.name.search("\n") != -1) {
      chrome.tabs.sendMessage(
        ...
        {
          method: "showMsg",
          msg: I18N.getMessage("Invalid_UserScript_name__Sry_"),
        },
        function (response) {},
      );
      return false;
      ...
      !o.clean &&
      oldscript &&
      script.version == oldscript.version &&
      !hashChanged
    ) {
      ...
      if (o.save) {
        msg +=
          I18N.getMessage("You_are_about_to_modify_a_UserScript_") + "     \n";
      } else {
        msg +=
          I18N.getMessage("You_are_about_to_reinstall_a_UserScript_") +
          "     \n";
        reset = true;
        msg +=
      ...
      !o.clean &&
      oldscript &&
      versionCmp(script.version, oldscript.version) == eOLDER
    ) {
      msg +=
        I18N.getMessage("You_are_about_to_downgrade_a_UserScript") + "     \n";
      msg += "\n" + I18N.getMessage("Name_") + "\n";
      msg +=
        "    " +
        script.name +
      ...
      msg +=
        I18N.getMessage("You_are_about_to_update_a_UserScript_") + "     \n";
      msg += "\n" + I18N.getMessage("Name_") + "\n";
      msg +=
        "    " +
        script.name +
      ...
      msg +=
        I18N.getMessage("You_are_about_to_install_a_UserScript_") + "     \n";
      msg += "\n" + I18N.getMessage("Name_") + "\n";
      msg +=
        "    " +
        script.name +
    ...
    if (!script.includes.length && !script.matches.length) {
      ...
      msg +=
        "    " +
        I18N.getMessage(
          "Tampermonkey_assumes_0urlAllHttp0_in_order_to_continue_",
          Helper.urlAllHttp,
        ) +
        "    \n";
    ...
    if (!script.options.awareOfChrome) {
      if (
        o.src.search("DOMContentLoaded") != -1
    ...
    var g =
      script.excludes.length + script.includes.length + script.matches.lenght;
    var c = 0;
    var m = 4;
    ...
    for (var h in script.options) {
      if (h.search("compat_") != -1 && script.options[h] === true) {
        compDe = true;
        break;
      }
    }
    ...
    if (compDe) {
      msg +=
        "\n" +
        I18N.getMessage(
          "Note__A_recheck_of_the_GreaseMonkey_FF_compatibility_options_may_be_required_in_order_to_run_this_script_",
        ) +
        "\n\n";
    }
    ...
    var doit = function () {
      ...
      if (!oldscript || o.clean)
        storeScriptStorage(script.name, { ts: new Date().getTime() });
      if (!o.cb) {
        notifyOptionsTab();
      }
    ...
    var inst = function (req) {
      if (req.readyState == 4) {
        if (req.status == 200) {
          ...
          if (V)
            console.log("scriptClick: " + url + " req.status = " + req.status);
          if (cb) cb(false, false);
        }
      }
    };
    ...
    var resp = function (tab) {
      ...
      if (id == 0) {
        ...
        chrome.tabs.create(
          {
            url:
              chrome.extension.getURL("ask.html") +
              "?i18n=" +
              Config.values.i18n,
          },
          created,
    ...
    getNotifyId: function (callback) {
      ...
      if (callback) {
        while (id == 0 || notify.responses[id] != undefined) {
          id = (
            new Date().getTime() + Math.floor(Math.random() * 6121983 + 1)
          ).toString();
        }
    ...
    notify: function (title, text, image, delay, perm, link, callback) {
      ...
      if (link) {
        args +=
          ...
          ";" +
          encodeURIComponent(link.src);
      }
      var notification = webkitNotifications.createHTMLNotification(
        "notification.html?" + args,
      );
      notification.show();
    },
  ...
  var ScriptUpdater = {
    check: function (force, showResult, id, callback) {
            ...
            try {
              ...
              if (Config.values.notification_silentScriptUpdate) {
                ...
                notify.show(
                  ...
                  msg,
                  chrome.extension.getURL("images/icon128.png"),
                  Config.values.scriptUpdateHideNotificationAfter,
                  install,
                );
              }
            } catch (e) {
      ...
      var prepare = function () {
        ...
        if (
          force ||
          new Date().getTime() - last.scripts >
            Config.values.scriptUpdateCheckPeriod
        ) {
          var exec = function () {
            runCheck();
    ...
    updateUserscripts: function (tabid, showResult, scriptid, callback) {
        ...
        (function () {
          ...
          var cb = function (req) {
            ...
            if (req.readyState == 4 && req.status == 200) {
              ...
              console.log(
                I18N.getMessage("UpdateCheck_of_0name0_Url_0url0_failed_", [
                  obj.r.script.name,
                  durl,
                ]),
              );
      ...
      var metaCheck = function (r) {
        ...
        var getmeta = function (o) {
          ...
          var hash_different =
            meta_found &&
            !!o.meta[cUSOHASH] &&
            o.meta[cUSOHASH] != r.script.hash;
          var version_found = meta_found && !!o.meta.version;
          var version_newer =
            version_found &&
            (!r.script.version ||
              versionCmp(o.meta.version, r.script.version) == eNEWER);

          // check script source in case:
          if (
            !meta_found || // no meta data was found
            hash_different || // hash has changed
            !version_found || // meta data does not contain version info
            version_newer
          ) {
            // we noticed a newer version

            if (V || UV)
              console.log(
                "bg: hash of script " +
                  r.script.name +
                  " has changed or does not exist! running version check!",
              );
            r.meta = o.meta;
            r.metasrc = o.metasrc;
      ...
      for (var k in names) {
        ...
        var c_disabled =
          !Config.values.scriptUpdateCheckDisabled &&
          !r.script.enabled &&
          !scriptid;
        ...
        if (
          c_scriptid ||
          c_disabled ||
          !(determineMetaURL(r.script) || determineSourceURL(r.script))
        )
          continue;
  ...
  var matchUrl = function (href, reg, match) {
    ...
    if (!match && reg.length > 1 && reg.substr(0, 1) == "/") {
      r = new RegExp(
  ...
  var loadScriptByName = function (name) {
    ...
    return { script: s, cond: TM_storage.getValue(name + condAppendix, null) };
  };
  ...
  var storeScript = function (name, script, triggerSync) {
    ...
    if (script) {
      ...
      TM_storage.setValue(name + condAppendix, {
        inc: script.includes,
    ...
    for (var k in old) {
      ...
      try {
        if (name && c.name == name) {
          ...
          if (V || SV)
            console.log("storage refresh/remove listeners of tab " + tabid);
        ...
        console.log(
          "Storage listener notification for script " +
            name +
            " failed! Page reload?!",
        );
      }
  ...
  var removeStorageListeners = function (name, id, send) {
    ...
    for (var k in old) {
      ...
      try {
        ...
        if (D)
          console.log(
            "Storage listener clear for script " +
              name +
              " failed! Page reload?!",
          );
      }
  ...
  var connectHandler = function (port) {
    ...
    var connectMsgHandler = function (request) {
      ...
      if (request.method == "xhr") {
        ...
        var cbe = function (req) {
          sendResponse({ error: true, data: req });
        };
        var cbs = function (req) {
          sendResponse({ success: true, data: req });
        };
        var cbc = function (req) {
          sendResponse({ change: true, data: req });
        };
        ...
        if (typeof sender.tab != "undefined") {
          if (V || SV)
            console.log(
              "storage add listener " + request.name + " " + request.id,
            );
          TM_storageListener.push({
            tabid: sender.tab.id,
            id: request.id,
            name: request.name,
          ...
          console.log(
            I18N.getMessage("Unable_to_load_storage_due_to_empty_tabID_"),
          );
        ...
        if (typeof sender.tab != "undefined") {
          if (V)
            console.log(
              "storage remove listener " + request.name + " " + request.id,
            );
        ...
        if (typeof sender.tab != "undefined") {
          ...
          console.log(
            I18N.getMessage("Unable_to_save_storage_due_to_empty_tabID_"),
          );
        }
        sendResponse({});
      }
  ...
  var requestHandling = {
    ...
    getTab: {
      ...
      exec: function (request, sender, sendResponse) {
        if (typeof sender.tab != "undefined") {
          if (typeof TM_tabs[sender.tab.id] == "undefined")
            TM_tabs[sender.tab.id] = { storage: {} };
          var tab = TM_tabs[sender.tab.id];
          sendResponse({ data: tab });
        } else {
          console.log(
            I18N.getMessage("Unable_to_deliver_tab_due_to_empty_tabID_"),
          );
    ...
    saveTab: {
      ...
      exec: function (request, sender, sendResponse) {
        if (typeof sender.tab != "undefined") {
          ...
          console.log(
            I18N.getMessage("Unable_to_save_tab_due_to_empty_tabID_"),
          );
        }
        sendResponse({});
      },
    ...
    buttonPress: {
      ...
      exec: function (request, sender, sendResponse) {
        ...
        if (request.name == "reset_simple") {
          ...
          var cb = function (ret) {
            clipboard.copy({
              content: Converter.UTF8.encode(ret.join("<br>")),
              type: "html",
            });
            done();
          };
          ...
          if (r.script && r.cond) {
            ...
            for (var k in dns.options) {
              ...
              if (typeof request[k] !== "undefined")
                r.script.options[k] = request[k];
            }
            for (var k in dns.options.override) {
              if (
                !dns.options.override.hasOwnProperty(k) ||
                k.search("merge_") == -1
              )
                continue;
            ...
            if (typeof request.enabled !== "undefined")
              r.script.enabled = request.enabled;
    ...
    modifyNativeScript: {
      allow: { extpage: true },
      exec: function (request, sender, sendResponse) {
        return requestHandling["modifyScriptOptions"].exec(
          request,
          sender,
          sendResponse,
        );
    ...
    saveScript: {
      ...
      exec: function (request, sender, sendResponse) {
        ...
        if (request.clean) {
          ...
          var callback = function (installed) {
            sendResponse({ installed: installed });
          };
    ...
    scriptClick: {
      ...
      exec: function (request, sender, sendResponse) {
        if (typeof sender.tab != "undefined") {
          var cb = function (found, installed) {
            ...
            if (found) {
              ...
              chrome.tabs.sendMessage(
                ...
                {
                  method: "showMsg",
                  msg: I18N.getMessage("Unable_to_get_UserScript__Sry_"),
                  id: request.id,
                },
                function (response) {},
              );
          ...
          console.log(
            I18N.getMessage("Unable_to_install_script_due_to_empty_tabID_"),
          );
        }
      },
    },
    ...
    getWebRequestInfo: {
      ...
      exec: function (request, sender, sendResponse) {
        if (typeof sender.tab != "undefined") {
          ...
          console.log(
            I18N.getMessage("Unable_to_run_scripts_due_to_empty_tabID_"),
          );
          sendResponse({});
        }
      },
    ...
    unLoad: {
      ...
      exec: function (request, sender, sendResponse) {
        if (
          !request.topframe && // unload of topframe will be handled by next load event
          ...
          if (V || UV)
            console.log("unload check " + contextId + " url: " + request.url);
          ...
          if (
            ...
            ctxRegistry.n[sender.tab.id].stats.running -=
              ctxRegistry.n[sender.tab.id].stats.executed[contextId].running;
            ctxRegistry.n[sender.tab.id].stats.disabled -=
              ctxRegistry.n[sender.tab.id].stats.executed[contextId].disabled;

            // shouldn't happen...
            if (ctxRegistry.n[sender.tab.id].stats.running < 0)
              ctxRegistry.n[sender.tab.id].stats.running = 0;
            if (ctxRegistry.n[sender.tab.id].stats.disabled < 0)
              ctxRegistry.n[sender.tab.id].stats.disabled = 0;
    ...
    prepare: {
      allow: { script: true },
      exec: function (request, sender, sendResponse) {
        if (typeof sender.tab != "undefined" && sender.tab.index >= 0) {
          // index of -1 is used by google search for omnibox
          var scheme =
            !sender.tab || !sender.tab.url || sender.tab.url.length < 4
              ? null
              : sender.tab.url.substr(0, 4);
          ...
          if (
            ...
            if (
              ...
              if (!scheme || (scheme != "http" && scheme != "file")) {
                ...
                var nfo = {
                  tabId: sender.tab.id,
                  frameId: request.topframe
                    ? 0
                    : 1 /* TODO: get frameId of sender!*/,
                  scriptId: request.id,
                  url: sender.tab.url,
                };

                ctxRegistry.setCache(
                  nfo.tabId,
                  nfo.frameId,
                  nfo.url,
                  Tab.prepare(nfo),
          ...
          var length_cb = function (enabledScriptsCount, disabledScriptsCount) {
            ...
            ctxRegistry.n[sender.tab.id].stats.executed[request.id] = {
              disabled: disabledScriptsCount,
              running: enabledScriptsCount,
            };
          ...
          if (
            Config.values.forbiddenPages.length == 0 ||
            validUrl(request.url, { exc: Config.values.forbiddenPages })
          ) {
    ...
    getFireItems: {
      ...
      exec: function (request, sender, sendResponse) {
        ...
        if (!TM_fire.isReady()) {
            ...
            action: TM_fire.status.action,
            state: TM_fire.status.progress,
          });
          return true;
        }
      ...
      if (entry) {
        if (entry.allow && entry.exec) {
          ...
          var extpage =
            reqId &&
            sender.tab &&
            sender.tab.url.search("chrome-extension") == 0;
          if (reqId && extpage) {
            var arr = sender.tab.url.match(
              new RegExp("chrome-extension://" + thisId + "/([a-zA-Z]*).html"),
            );
          ...
          if (
            ...
            if (D)
              console.log(
                "back: method " +
                  request.method +
                  " doesn't have the permission to be called from this context",
              );
            return false;
          }
        } else {
        ...
        console.log(
          "b: " + I18N.getMessage("Unknown_method_0name0", request.method),
        );
        return false;
      }
  ...
  var createFirePageItems = function (request, items) {
    ...
    ret.push({
      name: I18N.getMessage("Enable_Sort_Cache"),
    ...
    ret.push({
      name: I18N.getMessage("Available_Userscripts") + c,
      heading: true,
      scriptTab: true,
    });
    ...
    v +=
      I18N.getMessage("Entries_") +
      " " +
      (l.fire.entries ? l.fire.entries : "?") +
      "<br><br><br>";
    ...
    ret.push({
      name: I18N.getMessage("Check_for_Updates"),
  ...
  var createActionMenuItems = function (tab) {
    ...
    if (Config.values.fire_enabled) {
      s.push({
        name: I18N.getMessage("_0_scripts_found"),
    ...
    if (!s.length) {
      if (
        Config.values.forbiddenPages.length == 0 ||
        validUrl(url, { exc: Config.values.forbiddenPages })
      ) {
        s.push({
          name: I18N.getMessage("No_script_is_running"),
          image: chrome.extension.getURL("images/info.png"),
        });
      } else {
        s.push({
          ...
          image: chrome.extension.getURL("images/critical.png"),
        });
      }
    }
    s.push({
      name: I18N.getMessage("Get_new_scripts___"),
      image: chrome.extension.getURL("images/script_download.png"),
      url: "http://userscripts.org",
      newtab: true,
    });
    s.push({
      name: I18N.getMessage("Add_new_script___"),
      image: chrome.extension.getURL("images/script_add.png"),
      url: chrome.extension.getURL("options.html") + "?open=0",
      newtab: true,
    });
    ...
    c.push({
      name: I18N.getMessage("Check_for_userscripts_updates"),
      image: chrome.extension.getURL("images/update.png"),
      runUpdate: true,
    });
    c.push({
      name: I18N.getMessage("Report_a_bug"),
      image: chrome.extension.getURL("images/bug.png"),
      url: "http://tampermonkey.net/bug",
      newtab: true,
    });
    c.push({
      name: I18N.getMessage("Please_consider_a_donation"),
      image: chrome.extension.getURL("images/amor.png"),
      url: "http://tampermonkey.net/donate.html",
      newtab: true,
    });
    if (c.length) c.push(createDivider());
    c.push({
      name: I18N.getMessage("Options"),
      image: chrome.extension.getURL("images/agt_utilities.png"),
  ...
  var createOptionItems = function (cb) {
    ...
    ret.push({
      name: I18N.getMessage("Installed_userscripts"),
      heading: true,
      scriptTab: true,
    });
    ...
    if (!s.length) {
      s.push({
        name: I18N.getMessage("No_script_is_installed"),
        image: chrome.extension.getURL("images/info.png"),
      });
      s.push({
        name: I18N.getMessage("Get_some_scripts___"),
    ...
    var done = function (exts) {
      ...
      ret.push({
        name: I18N.getMessage("New_userscript"),
        id: null,
        image: chrome.extension.getURL("images/script_add.png"),
      ...
      optsg.push({
        name: I18N.getMessage("Config_Mode"),
        id: "configMode",
        level: 0,
        option: true,
        select: [
          { name: I18N.getMessage("Novice"), value: 0 },
      ...
      optsg.push({
        ...
        select: [
          ...
          {
            name: I18N.getOriginalMessage("Chinese__Simplified_"),
            value: "zh_CN",
          },
          {
            name: I18N.getOriginalMessage("Chinese__Traditional_"),
            value: "zh_TW",
          },
      ...
      optsg.push({
        name: I18N.getMessage("Make_includes_more_safe"),
        id: "safeUrls",
        level: 60,
        option: true,
        checkbox: true,
        enabled: Config.values.safeUrls,
        desc: I18N.getMessage("Includes_more_safe_example"),
      });
      optsg.push({
        name: I18N.getMessage("Fix_includes"),
        id: "tryToFixUrl",
        level: 60,
        option: true,
        checkbox: true,
        enabled: Config.values.tryToFixUrl,
        desc: I18N.getMessage("Fix_includes_example"),
      });
      optsg.push({
        name: I18N.getMessage("Auto_reload_on_script_enabled"),
        level: 20,
        id: "autoReload",
        option: true,
        checkbox: true,
        enabled: Config.values.autoReload,
        desc: I18N.getMessage("Auto_reload_on_script_enabled_desc"),
      });

      optsg.push({
        name: I18N.getMessage("Debug_scripts"),
        level: 100,
        id: "debug",
        option: true,
        checkbox: true,
        enabled: Config.values.debug,
        desc: "",
      });
      optsg.push({
        name: I18N.getMessage("Show_fixed_source"),
        level: 100,
        id: "showFixedSrc",
        option: true,
        checkbox: true,
        enabled: Config.values.showFixedSrc,
        desc: "",
      });
      optsg.push({
        name: "LogLevel",
      ...
      optsy.push({
        name: I18N.getMessage("TESLA") + " BETA",
        section: true,
        level: 50,
        needsave: true,
      });
      ...
      optsy.push({
        name: I18N.getMessage("Enable_TESLA"),
        id: "sync_enabled",
        level: 50,
        option: true,
        checkbox: true,
      ...
      optsy.push({
        name: I18N.getMessage("Sync_Type"),
        id: "sync_type",
        enabler: true,
        level: 50,
        option: true,
        select: [
          { name: "pastebin.com", value: SyncInfo.types.ePASTEBIN },
          {
            name: "Chrome Sync (Beta)",
            value: SyncInfo.types.eCHROMESYNC,
            enable: { sync_id: 0, create_tesla_data: 0 },
          },
        ],
      ...
      optsy.push({
        name: I18N.getMessage("Create_Exportable_Data"),
      ...
      optsa.push({
        name: I18N.getMessage("Appearance"),
        section: true,
        level: 20,
      });

      optsa.push({
        name: I18N.getMessage("Update_Notification"),
      ...
      optsa.push({
        name: I18N.getMessage("Icon_badge_info"),
        id: "appearance_badges",
        level: 50,
        option: true,
        select: [
          { name: I18N.getMessage("Off"), value: "off" },
          { name: I18N.getMessage("Running_scripts"), value: "running" },
          {
            name: I18N.getMessage("Unique_running_scripts"),
      ...
      opttf.push({
        name: I18N.getMessage("Enable_TamperFire"),
        id: "fire_enabled",
        level: 0,
        option: true,
        checkbox: true,
      ...
      opttf.push({
        name: I18N.getMessage("Update_interval"),
        id: "fire_updatePeriod",
        level: 50,
        option: true,
        select: [
          { name: I18N.getMessage("Never"), value: 0 },
          { name: I18N.getMessage("Every_Day"), value: 24 * 60 * 60 * 1000 },
          {
            name: I18N.getMessage("Every_Week"),
            value: 7 * 24 * 60 * 60 * 1000,
          },
          {
            name: I18N.getMessage("Every_2_Weeks"),
            value: 14 * 24 * 60 * 60 * 1000,
          },
          {
            name: I18N.getMessage("Every_Month"),
      ...
      optse.push({
        name: I18N.getMessage("Enable_Editor"),
        id: "editor_enabled",
        level: 100,
        option: true,
        checkbox: true,
      ...
      optse.push({
        name: I18N.getMessage("Key_Mapping"),
        id: "editor_keyMap",
        level: 50,
        option: true,
        reload: true,
        warning: I18N.getMessage("A_reload_is_required"),
        select: [
          { name: I18N.getMessage("Windows"), value: "windows" },
      ...
      optse.push({
        name: I18N.getMessage("Indentation_Width"),
        id: "editor_indentUnit",
        level: 50,
        option: true,
        select: [
          { name: I18N.getMessage("1"), value: 1 },
      ...
      optse.push({
        name: I18N.getMessage("Indent_with"),
        id: "editor_indentWithTabs",
        level: 50,
        option: true,
        select: [
          { name: I18N.getMessage("Tabs"), value: "tabs" },
      ...
      optse.push({
        name: I18N.getMessage("TabMode"),
        id: "editor_tabMode",
        level: 50,
        option: true,
        select: [
          { name: I18N.getMessage("Classic"), value: "classic" },
      ...
      optse.push({
        name: I18N.getMessage("Reindent_on_typing"),
      ...
      optse.push({
        name: I18N.getMessage("Show_Line_Numbers"),
      ...
      optse.push({
        name: I18N.getMessage("Enable_easySave"),
        id: "editor_easySave",
        level: 20,
        option: true,
        checkbox: true,
      ...
      optsu.push({
        name: I18N.getMessage("Script_Update"),
        section: true,
        level: 0,
      });

      optsu.push({
        name: I18N.getMessage("Check_disabled_scripts"),
      ...
      optsu.push({
        name: I18N.getMessage("Check_interval"),
        ...
        select: [
          ...
          {
            name: I18N.getMessage("Every_12_Hour"),
            value: 12 * 60 * 60 * 1000,
          },
          { name: I18N.getMessage("Every_Day"), value: 24 * 60 * 60 * 1000 },
          {
      ...
      optsu.push({
        name: I18N.getMessage("Dont_ask_me_for_simple_script_updates"),
      ...
      optsu.push({
        name: I18N.getMessage("Hide_notification_after"),
      ...
      optss.push({
        name: I18N.getMessage("Security"),
        section: true,
        level: 50,
      });

      optss.push({
        name: I18N.getMessage("Allow_overwrite_javascript_settings"),
        id: "scriptblocker_overwrite",
        level: 50,
        option: true,
        select: [
          { name: I18N.getMessage("Yes"), value: "yes" },
          { name: I18N.getMessage("No"), value: "no" },
        ],
        value: Config.values.scriptblocker_overwrite,
        desc: I18N.getMessage(
          "Tampermonkey_can_not_work_when_javascript_is_disabled",
        ),
      });

      optss.push({
        name: I18N.getMessage("Add_TM_to_CSP"),
      ...
      optss.push({
        name: I18N.getMessage("Allow_headers_to_be_modified_by_scripts"),
      ...
      optss.push({
        name: I18N.getMessage("Forbidden_Pages"),
        id: "forbiddenPages",
        level: 50,
        option: true,
        input: true,
      ...
      optss.push({
        name: I18N.getMessage("_require_blacklist"),
      ...
      optns.push({
        name: I18N.getMessage("Userscripts"),
        section: true,
        level: 80,
      });

      optns.push({
        name: I18N.getMessage("New_script_template_"),
        id: "scriptTemplate",
        level: 80,
        option: true,
        input: true,
      ...
      optsr.push({
        name: I18N.getMessage("Reset_Section"),
        section: true,
        level: 50,
      });

      optsr.push({
        name: I18N.getMessage("Restart_Tampermonkey"),
        id: "reset_simple",
        level: 50,
        button: true,
        reload: true,
      ...
      optsr.push({
        name: I18N.getMessage("Factory_Reset"),
        id: "reset_factory",
        level: 80,
        button: true,
        reload: true,
      ...
      if (storagePermission.hasPermission) {
        optsr.push({
          name: I18N.getMessage("Chrome_Sync_Reset"),
          ...
          value: 0,
          warning: I18N.getMessage(
            "This_will_remove_all_stored_data_from_google_sync_Ok_",
          ),
        });
      }
      ...
      if (false) {
        ret.push({
          name: I18N.getMessage("Registered_menu_cmds"),
          heading: true,
        });
  ...
  var convertMenuCmdsToMenuItems = function (tabId) {
    ...
    for (var k in arr) {
      ...
      var item = {
        name: c.name,
        id: c.id,
        image: chrome.extension.getURL("images/package_utilities.png"),
        menucmd: true,
      };
      ret.push(item);
    }
  ...
  var convertScriptsToMenuItems = function (scripts, options) {
    ...
    for (var k in scripts) {
      ...
      if (!script.icon64 && !script.icon) {
        item.icon64 = chrome.extension.getURL(
          item.user_agent ? "images/user_agent.png" : "images/txt.png",
        );
      }
  ...
  var convertMgmtToMenuItems = function (tab, options) {
    ...
    if (tab) {
      if (!ctxRegistry.isEmpty(tab.id)) {
        var it = function (i, v) {
          if (V || UV)
            console.log("Found at ctxRegistry[" + tab.id + "].urls -> " + i);
  ...
  var permission = {
    ...
    get: function (cb) {
      var gotPerms = function (p) {
        Helper.forEach(p.permissions, function (v, k) {
          permission.permissions[v] = true;
        });
  ...
  var contentSettings = {
    ...
    init: function () {
      var g = function (s) {
        ...
        contentSettings.runCheck =
          contentSettings.hasPermission &&
          Config.values.scriptblocker_overwrite == "yes";
        if (D)
          console.log(
            "bg: contentSettings: runCheck = " +
              contentSettings.runCheck +
              " hasPerm = " +
              contentSettings.hasPermission,
          );
    ...
    askForPermission: function (cb) {
      permission.ask(
        permission.permContentSettings,
        I18N.getMessage("A_script_blocker_was_detected_"),
        I18N.getMessage(
          "Click_here_to_allow_TM_to_override_the_script_blocker",
        ),
        cb,
      );
    },
  ...
  var initBrowserAction = function () {
    chrome.browserAction.setIcon({
      path: chrome.extension.getURL("images/icon_grey.png"),
    });
  ...
  var webRequest = {
    ...
    headerCheck: function (details) {
      if (details.tabId >= 0 && _webRequest.verified == false) {
        if (D || UV)
          console.log(
            "bg: verify that webRequest is working at " +
              details.type +
              " to " +
              details.url,
          );
        ...
        if (D)
          console.log(
            "bg: verified webRequest " +
              (_webRequest.headers ? "" : "not ") +
              "being working",
          );

        try {
          chrome.webRequest.onSendHeaders.removeListener(
            webRequest.headerCheck,
          );
        } catch (ex) {
      ...
      for (var i = 0; i < rh.length; i++) {
        ...
        if (xmlreq && item.name == "Location") {
          var wrap = function () {
            ...
            webRequest.redirects[rid] = {
              url: item.value,
              to: window.setTimeout(cleanRedirect, 10000),
            };
          };
          wrap();
          break;
        } else if (
          Config.values.webrequest_fixCSP &&
          (item.name == "X-WebKit-CSP" ||
            item.name == "X-Content-Security-Policy")
        ) {
          var n = item.value.replace(
            /script-src /,
            "script-src " +
              "chrome-extension://" +
              chrome.extension.id +
          ...
          if (D)
            console.log('csp: replace "' + item.value + '" with "' + n + '"');
    ...
    headerFix: function (details) {
      ...
      if (main) {
        ...
        if (V || UV || EV)
          console.log(
            "bg: create new ctxRegistry entry for URL " + details.url,
          );
        var nfo = {
          tabId: details.tabId,
          frameId: 0 /* 'main_frame' */,
          scriptId: 0,
          url: details.url,
        };
        ...
        if (V || UV || EV) {
          var later = function () {
            ...
            chrome.contentSettings.javascript.get(
              { primaryUrl: details.url },
              cb,
            );
          };
          console.log(
            "contentSettings: (" + new Date().getTime() + ") allow URL " + pat,
          );
          later();
      ...
      if (u) {
        for (var k in ctxRegistry.n[details.tabId].user_agent) {
          if (!ctxRegistry.n[details.tabId].user_agent.hasOwnProperty(k))
            continue;
          uv = ctxRegistry.n[details.tabId].user_agent[k];
        }
        if (V || UV)
          console.log("bg: userscript user-agent spoof enabled! (" + uv + ")");
      }
    ...
    checkRequestForUserscript: function (details) {
      ...
      if (
        details.tabId > 0 &&
        details.type ==
          "main_frame" /* ignore URLs from frames, xmlhttprequest, ... */ &&
        ...
        var url =
          chrome.extension.getURL("ask.html") +
          "?script=" +
          Converter.Base64.encode(details.url) +
          "&i18n=" +
          Config.values.i18n;
        if (RV)
          console.log(
            "bg: user script detected @ " +
              details.url +
              " -> open tab with " +
              url,
          );
      ...
      if (_webRequest.use) {
        try {
          chrome.webRequest.onBeforeRequest.removeListener(
            webRequest.checkRequestForUserscript,
          );
          chrome.webRequest.onBeforeSendHeaders.removeListener(
            webRequest.headerFix,
          );
          chrome.webRequest.onHeadersReceived.removeListener(
            webRequest.detectRedirect,
          );
    ...
    init: function (verified, headers) {
      if (_webRequest.use) {
        try {
          ...
          _webRequest.testprefix =
            _webRequest.prefix +
            Math.floor(Math.random() * 6121983 + 1).toString();
  ...
  var loadListener = function (tabID, changeInfo, tab) {
    if (!Config.initialized) {
      window.setTimeout(function () {
        loadListener(tabID, changeInfo, tab);
      }, 100);
      return;
    }
    ...
    var sere = function () {
      ...
      chrome.tabs.sendMessage(tabID, { method: "getSrc" }, function (response) {
  ...
  var tabUpdateListener = function (
    tabId,
    changeInfo,
    tab,
    request,
  ...
  var initObjects = function () {
    ...
    if (Config.values.sync_enabled && Config.values.sync_type) {
      SyncClient.enable();
    ...
    if (Config.values.webrequest_use != "no") {
      ...
      webRequest.init(
        Config.values.webrequest_modHeaders != "auto",
