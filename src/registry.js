...
var Registry = {
  ...
  init: function () {},
  onLoad: function () {
    const cb = Registry.callbacks.pop();
    if (cb) cb();
  },
  checkLoading: function () {
    let l = false;
    for (const k in Registry.objects) {
  ...
  register: function (name, obj, overwrite) {
    if (RD || RV || V) {
      console.log("Registry.register " + name + " overwrite: " + overwrite);
    }
  ...
  registerRaw: function (name, obj, overwrite) {
    if (RD || RV || V) {
      console.log("Registry.registerRaw " + name + " overwrite: " + overwrite);
    }
