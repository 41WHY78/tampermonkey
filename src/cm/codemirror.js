...
window.CodeMirror = (function () {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    for (const opt in defaults) {
      if (Object.prototype.hasOwnProperty.call(defaults, opt)) {
        options[opt] = (
          givenOptions &&
          Object.prototype.hasOwnProperty.call(givenOptions, opt)
            ? givenOptions
            : defaults
        )[opt];
      }
    ...
    const scrollbarInner = elt("div", null, "CodeMirror-scrollbar-inner");
    // The vertical scrollbar. Horizontal scrolling is handled by the scroller itself.
    const scrollbar = elt("div", [scrollbarInner], "CodeMirror-scrollbar");
    // DIVs containing the selection and the actual code
    const lineDiv = elt("div");
    const selectionDiv = elt(
      "div",
      null,
      null,
    ...
    const cursor = elt("pre", "\u00a0", "CodeMirror-cursor");
    const widthForcer = elt(
      "pre",
      "\u00a0",
      "CodeMirror-cursor",
      "visibility: hidden",
    ...
    const gutterText = elt("div", null, "CodeMirror-gutter-text");
    const gutter = elt("div", [gutterText], "CodeMirror-gutter");
    ...
    const sizer = elt("div", [mover], null, "position: relative");
    // Provides scrolling
    const scroller = elt("div", [sizer], "CodeMirror-scroll");
    ...
    const poll = new Delayed();
    const highlight = new Delayed();
    let blinker;
    ...
    let mode;
    const doc = new BranchChunk([new LeafChunk([new Line("")])]);
    let frontier = 0;
    let focused;
    loadMode();
    ...
    // marked.
    let bracketHighlighted;
    // Tracks the maximum line length so that the horizontal scrollbar
    // can be kept static when scrolling.
    let maxLine = getLine(0);
    let updateMaxLine = false;
    let maxLineChanged = true;
    let pollingFast = false; // Ensures slowPoll doesn't cancel fastPoll
    let goalColumn = null;
    ...
    var instance = (wrapper.CodeMirror = {
      ...
      getHistory: function () {
        function cp(arr) {
          for (var i = 0, nw = [], nwelt; i < arr.length; ++i) {
            nw.push((nwelt = []));
            for (let j = 0, elt = arr[i]; j < elt.length; ++j) {
              const old = [];
              const cur = elt[j];
              nwelt.push({ start: cur.start, added: cur.added, old });
              for (let k = 0; k < cur.old.length; ++k) {
      ...
      setSelection: operation(function (from, to, user) {
        (user ? setSelectionUser : setSelection)(
          clipPos(from),
          clipPos(to || from),
        );
      }),
      ...
      setLine: operation(function (line, text) {
        if (isLine(line)) {
          replaceRange(
            text,
            { line, ch: 0 },
            { line, ch: getLine(line).text.length },
          );
        }
      }),
      removeLine: operation(function (line) {
        if (isLine(line)) {
          replaceRange("", { line, ch: 0 }, clipPos({ line: line + 1, ch: 0 }));
        }
      }),
      ...
      execCommand: function (cmd, arg) {
        if (instance.usercommands[cmd]) {
          return instance.usercommands[cmd](instance, arg);
        } else {
          return commands[cmd](instance, arg);
        }
      },
      // Stuff used by commands, probably not much use to outside code.
      setCommand: function (cmd, fn) {
        instance.usercommands[cmd] = fn;
      },
      ...
      toggleOverwrite: function () {
        if (overwrite) {
          overwrite = false;
          cursor.className = cursor.className.replace(
    ...
    function updateLineHeight(line, height) {
      ...
      for (let n = line; n; n = n.parent) n.height += diff;
    }
    ...
    function onMouseDown(e) {
      ...
      const startstart = sel.from;
      const startend = sel.to;

      function doSelect(cur) {
        if (type == "single") {
          setSelectionUser(start, cur);
        } else if (type == "double") {
          const word = findWordAt(cur);
          if (posLess(cur, startstart)) setSelectionUser(word.from, startend);
          else setSelectionUser(startstart, word.to);
        } else if (type == "triple") {
          if (posLess(cur, startstart)) {
            setSelectionUser(startend, clipPos({ line: cur.line, ch: 0 }));
          } else {
            setSelectionUser(
              startstart,
              clipPos({ line: cur.line + 1, ch: 0 }),
            );
          }
        }
      }
    ...
    function onDoubleClick(e) {
      for (let n = e_target(e); n != wrapper; n = n.parentNode) {
        ...
        const loadFile = function (file, i) {
          ...
          reader.onload = function () {
            ...
            if (++read == n) {
              pos = clipPos(pos);
              operation(function () {
                const end = replaceRange(text.join(""), pos, pos);
    ...
    function doHandleBinding(bound, dropShift) {
      if (typeof bound === "string") {
        bound = instance.usercommands[bound] || commands[bound];
        if (!bound) return false;
      }
      const prevShift = shiftSelecting;
      try {
    ...
    function handleKeyBinding(e) {
      ...
      const startMap = getKeyMap(options.keyMap);
      const next = startMap.auto;
      ...
      let name = keyNames[e_prop(e, "keyCode")];
      let handled = false;
      const flipCtrlCmd = opera && mac;
    ...
    function onKeyPress(e) {
      ...
      const keyCode = e_prop(e, "keyCode");
      const charCode = e_prop(e, "charCode");
    ...
    function onBlur() {
      if (focused) {
        ...
        scroller.className = scroller.className.replace(
          " CodeMirror-focused",
          "",
        );
      }
    ...
    function updateLinesNoUndo(from, to, lines, selFrom, selTo) {
      ...
      let maxLineLength = maxLine.text.length;
      ...
      if (options.lineWrapping) {
        const perLine = Math.max(5, scroller.clientWidth / charWidth() - 3);
        doc.iter(from.line, from.line + lines.length, function (line) {
          if (line.hidden) return;
          const guess = Math.ceil(line.text.length / perLine) || 1;
      ...
      const lendiff = lines.length - nlines - 1;
      ...
      if (options.onChange) {
        ...
        const changeObj = { from, to, text: lines };
        if (textChanged) {
    ...
    function needsScrollbar() {
      const realHeight = doc.height * textHeight() + 2 * paddingTop();
      return realHeight * 0.99 > scroller.offsetHeight ? realHeight : false;
    }

    function updateVerticalScroll(scrollTop) {
      const scrollHeight = needsScrollbar();
      scrollbar.style.display = scrollHeight ? "block" : "none";
      if (scrollHeight) {
        scrollbarInner.style.height = sizer.style.minHeight =
          scrollHeight + "px";
    ...
    function replaceRange(code, from, to) {
      ...
      function adjustPos(pos) {
        ...
        const line = pos.line + code.length - (to.line - from.line) - 1;
        let ch = pos.ch;
        if (pos.line == to.line) {
          ch +=
            lst(code).length - (to.ch - (to.line == from.line ? from.ch : 0));
        }
        return { line, ch };
      }
      let end;
    ...
    function replaceRange1(code, from, to, computeSel) {
      const endch =
        code.length == 1 ? code[0].length + from.ch : lst(code).length;
      const newSel = computeSel({
    ...
    function getRange(from, to, lineSep) {
      ...
      const code = [getLine(l1).text.slice(from.ch)];
    ...
    function scrollCursorIntoView() {
      ...
      if (doScroll != null) {
        const hidden = cursor.style.display == "none";
        if (hidden) {
    ...
    function calculateCursorCoords() {
      const cursor = localCoords(sel.inverted ? sel.from : sel.to);
      const x = options.lineWrapping
        ? Math.min(cursor.x, lineSpace.offsetWidth)
        : cursor.x;
      return { x, y: cursor.y, yBot: cursor.yBot };
    }
    function scrollIntoView(x1, y1, x2, y2) {
      const scrollPos = calculateScrollPos(x1, y1, x2, y2);
    ...
    function calculateScrollPos(x1, y1, x2, y2) {
      ...
      const screentop = scrollbar.scrollTop;
      const result = {};
      const docBottom = needsScrollbar() || Infinity;
      const atTop = y1 < pt + 10;
      const atBottom = y2 + pt > docBottom - 10;
      ...
      const screenleft = scroller.scrollLeft;
      const gutterw = options.fixedGutter ? gutter.clientWidth : 0;
      const atLeft = x1 < gutterw + pl + 10;
    ...
    function visibleLines(scrollTop) {
      const lh = textHeight();
      const top =
        (scrollTop != null ? scrollTop : scrollbar.scrollTop) - paddingTop();
      const fromHeight = Math.max(0, Math.floor(top / lh));
      const toHeight = Math.ceil((top + scroller.clientHeight) / lh);
      return {
    ...
    function updateDisplay(changes, suppressCallback, scrollTop) {
      ...
      const visible = visibleLines(scrollTop);
      ...
      let from = Math.max(visible.from - 100, 0);
      let to = Math.min(doc.size, visible.to + 100);
      ...
      if (
        from != showingFrom ||
        (to != showingTo && options.onViewportChange)
      ) {
        setTimeout(function () {
      ...
      function checkHeights() {
        let curNode = lineDiv.firstChild;
        ...
        doc.iter(showingFrom, showingTo, function (line) {
          ...
          if (!curNode) return;
          if (!line.hidden) {
            const height = Math.round(curNode.offsetHeight / th) || 1;
    ...
    function computeIntact(intact, changes) {
      for (let i = 0, l = changes.length || 0; i < l; ++i) {
        const change = changes[i];
        const intact2 = [];
        const diff = change.diff || 0;
        for (let j = 0, l2 = intact.length; j < l2; ++j) {
          const range = intact[j];
    ...
    function updateGutter() {
      ...
      const hText = mover.offsetHeight;
      const hEditor = scroller.clientHeight;
      ...
      doc.iter(
        showingFrom,
        Math.max(showingTo, showingFrom + 1),
        function (line) {
          if (line.hidden) {
            fragment.appendChild(elt("pre"));
          } else {
            const marker = line.gutterMarker;
      ...
      if (normalNode != null && options.lineNumbers) {
        const node = gutterText.childNodes[normalNode - showingFrom];
        const minwidth = String(doc.size).length;
        const val = eltText(node.firstChild);
        let pad = "";
    ...
    function updateSelection() {
      const collapsed = posEq(sel.from, sel.to);
      const fromPos = localCoords(sel.from, true);
      const toPos = collapsed ? fromPos : localCoords(sel.to, true);
      const headPos = sel.inverted ? fromPos : toPos;
      const th = textHeight();
      const wrapOff = eltOffset(wrapper);
      const lineOff = eltOffset(lineDiv);
      inputDiv.style.top =
        Math.max(
          0,
          Math.min(
            scroller.offsetHeight,
            headPos.y + lineOff.top - wrapOff.top,
          ),
        ) + "px";
      inputDiv.style.left =
        Math.max(
          ...
          Math.min(
            ...
            headPos.x + lineOff.left - wrapOff.left,
          ),
        ) + "px";
      if (collapsed) {
        ...
        const sameLine = fromPos.y == toPos.y;
        const fragment = document.createDocumentFragment();
        const clientWidth = lineSpace.clientWidth || lineSpace.offsetWidth;
        const clientHeight = lineSpace.clientHeight || lineSpace.offsetHeight;
        const add = function (left, top, right, height) {
          const rstyle = quirksMode
            ? "width: " +
        ...
        if (sel.from.ch && fromPos.y >= 0) {
          const right = sameLine ? clientWidth - toPos.x : 0;
          add(fromPos.x, fromPos.y, right, th);
        }
        const middleStart = Math.max(0, fromPos.y + (sel.from.ch ? th : 0));
        const middleHeight = Math.min(toPos.y, clientHeight) - middleStart;
    ...
    function setSelectionUser(from, to) {
      const sh = shiftSelecting && clipPos(shiftSelecting);
      if (sh) {
    ...
    function setSelection(from, to, oldFrom, oldTo) {
      ...
      if (from.line != oldFrom) {
        const from1 = skipHidden(from, oldFrom, sel.from.ch);
      ...
      if (options.autoClearEmptyLines && posEq(sel.from, sel.to)) {
        const head = sel.inverted ? from : to;
        if (head.line != sel.from.line && sel.from.line < doc.size) {
          const oldLine = getLine(sel.from.line);
          if (/^\s+$/.test(oldLine.text)) {
            setTimeout(
              operation(function () {
                if (oldLine.parent && /^\s+$/.test(oldLine.text)) {
                  const no = lineNo(oldLine);
                  replaceRange(
                    "",
                    { line: no, ch: 0 },
                    { line: no, ch: oldLine.text.length },
                  );
                }
              }, 10),
            );
    ...
    function skipHidden(pos, oldLine, oldCh) {
      function getNonHidden(dir) {
        let lNo = pos.line + dir;
        const end = dir == 1 ? doc.size : -1;
        while (lNo != end) {
          const line = getLine(lNo);
          if (!line.hidden) {
            let ch = pos.ch;
    ...
    function setCursor(line, ch, user) {
      const pos = clipPos({ line, ch: ch || 0 });
    ...
    function clipPos(pos) {
      ...
      const ch = pos.ch;
      const linelen = getLine(pos.line).text.length;
    ...
    function findPosH(dir, unit) {
      const end = sel.inverted ? sel.from : sel.to;
      let line = end.line;
      let ch = end.ch;
      let lineObj = getLine(line);
      function findNextLine() {
        for (
          let l = line + dir, e = dir < 0 ? -1 : doc.size;
          l != e;
          l += dir
        ) {
          const lo = getLine(l);
    ...
    function moveV(dir, unit) {
      const dist = 0;
      const pos = localCoords(sel.inverted ? sel.from : sel.to, true);
    ...
    function findWordAt(pos) {
      const line = getLine(pos.line).text;
      let start = pos.ch;
      let end = pos.ch;
      if (line) {
        ...
        const check = isWordChar(startChar)
          ? isWordChar
          : /\s/.test(startChar)
            ? function (ch) {
                return /\s/.test(ch);
    ...
    function selectLine(line) {
      setSelectionUser({ line, ch: 0 }, clipPos({ line: line + 1, ch: 0 }));
    }
    function indentSelected(mode) {
      if (posEq(sel.from, sel.to)) return indentLine(sel.from.line, mode);
      const e = sel.to.line - (sel.to.ch ? 0 : 1);
      for (let i = sel.from.line; i <= e; ++i) indentLine(i, mode);
    }
    ...
    function indentLine(n, how) {
      ...
      const curSpace = line.indentation(options.tabSize);
      ...
      const diff = indentation - curSpace;

      let indentString = "";
      let pos = 0;
      if (options.indentWithTabs) {
        for (let i = Math.floor(indentation / options.tabSize); i; --i) {
          pos += options.tabSize;
          indentString += "\t";
        }
      }
    ...
    function gutterChanged() {
      const visible = options.gutter || options.lineNumbers;
    ...
    function wrappingChanged(from, to) {
      if (options.lineWrapping) {
        ...
        const perLine = scroller.clientWidth / charWidth() - 3;
        doc.iter(0, doc.size, function (line) {
          if (line.hidden) return;
          const guess = Math.ceil(line.text.length / perLine) || 1;
    ...
    function keyMapChanged() {
      const style = keyMap[options.keyMap].style;
      wrapper.className =
    ...
    function setBookmark(pos) {
      pos = clipPos(pos);
      const marker = new TextMarker("bookmark");
      const line = getLine(pos.line);
      history.addChange(
        pos.line,
        1,
        [newHL(line.text, line.markedSpans)],
        true,
      );
      const span = { from: pos.ch, to: pos.ch, marker };
      line.markedSpans = (line.markedSpans || []).concat([span]);
      marker.lines.push(line);
      return marker;
    }
    ...
    function findMarksAt(pos) {
      pos = clipPos(pos);
      const markers = [];
      const spans = getLine(pos.line).markedSpans;
      if (spans) {
    ...
    function addGutterMarker(line, text, className) {
      ...
      line.gutterMarker = { text, style: className };
      gutterDirty = true;
      return line;
    }
    ...
    function setLineHidden(handle, hidden) {
      return changeLine(handle, function (line, no) {
        if (line.hidden != hidden) {
          ...
          const fline = sel.from.line;
          const tline = sel.to.line;
          if (hidden && (fline == no || tline == no)) {
            const from =
              fline == no
                ? skipHidden({ line: fline, ch: 0 }, fline, 0)
                : sel.from;
            const to =
              tline == no
                ? skipHidden({ line: tline, ch: 0 }, tline, 0)
                : sel.to;
    ...
    function localCoords(pos, inLineWrap) {
      ...
      if (pos.ch == 0) x = 0;
        ...
        const sp = measureLine(getLine(pos.line), pos.ch);
        x = sp.left;
        if (options.lineWrapping) y += Math.max(0, sp.top);
      }
      return { x, y, yBot: y + lh };
    }
    // Coords must be lineSpace-local
    function coordsChar(x, y) {
      const th = textHeight();
      const cw = charWidth();
      const heightPos = displayOffset + Math.floor(y / th);
      if (heightPos < 0) return { line: 0, ch: 0 };
      const lineNo = lineAtHeight(doc, heightPos);
      if (lineNo >= doc.size) {
        return { line: doc.size - 1, ch: getLine(doc.size - 1).text.length };
      }
      const lineObj = getLine(lineNo);
      const text = lineObj.text;
      const tw = options.lineWrapping;
      const innerOff = tw ? heightPos - heightAtLine(doc, lineNo) : 0;
      if (x <= 0 && innerOff == 0) return { line: lineNo, ch: 0 };
      let wrongLine = false;
      function getX(len) {
        const sp = measureLine(lineObj, len);
        if (tw) {
          const off = Math.round(sp.top / th);
      ...
      for (;;) {
        if (to - from <= 1) {
          const after = x - fromX < toX - x;
          return { line: lineNo, ch: after ? from : to, after };
        }
        const middle = Math.ceil((from + to) / 2);
        const middleX = getX(middle);
        if (middleX > x) {
          to = middle;
          toX = middleX;
    ...
    function pageCoords(pos) {
      const local = localCoords(pos, true);
      const off = eltOffset(lineSpace);
      return {
        x: off.left + local.x,
        y: off.top + local.y,
    ...
    let cachedHeight, cachedHeightFor, measurePre;
    function textHeight() {
      if (measurePre == null) {
        measurePre = elt("pre");
        for (let i = 0; i < 49; ++i) {
      ...
      const offsetHeight = lineDiv.clientHeight;
    ...
    let cachedWidth;
    let cachedWidthFor = 0;
    function charWidth() {
      if (scroller.clientWidth == cachedWidthFor) return cachedWidth;
      cachedWidthFor = scroller.clientWidth;
      const anchor = elt("span", "x");
    ...
    function posFromMouse(e, liberal) {
      const offW = eltOffset(scroller, true);
      let x;
      let y;
      ...
      const offL = eltOffset(lineSpace, true);
      return coordsChar(x - offL.left, y - offL.top);
    }
    let detectingSelectAll;
    function onContextMenu(e) {
      ...
      const oldCSS = input.style.cssText;
    ...
    function matchBrackets(autoclear) {
      const head = sel.inverted ? sel.from : sel.to;
      var line = getLine(head.line);
      let pos = head.ch - 1;
      const match =
      ...
      const forward = match.charAt(1) == ">";
      const d = forward ? 1 : -1;
      const st = line.styles;
      ...
      const stack = [line.text.charAt(pos)];
      const re = /[(){}[\]]/;
      function scan(line, from, to) {
        ...
        let cur;
        for (
          let i = forward ? 0 : st.length - 2, e = forward ? st.length : -2;
          i != e;
          i += 2 * d
        ) {
          const text = st[i];
          ...
          for (
            let j = forward ? 0 : text.length - 1,
              te = forward ? text.length : -1;
            j != te;
            j += d, pos += d
          ) {
            if (pos >= from && pos < to && re.test((cur = text.charAt(j)))) {
              const match = matching[cur];
    ...
    function findStartLine(n) {
      let minindent, minline;
      for (let search = n, lim = n - 40; search > lim; --search) {
        if (search == 0) return 0;
        const line = getLine(search - 1);
        if (line.stateAfter) return search;
        const indented = line.indentation(options.tabSize);
    ...
    function getStateBefore(n) {
      const pos = findStartLine(n);
      let state = pos && getLine(pos - 1).stateAfter;
    ...
    function endOperation() {
      ...
      if (maxLineChanged && !options.lineWrapping) {
        const cursorWidth = widthForcer.offsetWidth;
        const left = measureLine(maxLine, maxLine.text.length).left;
        if (!ie_lt8) {
      ...
      const sc = selectionChanged;
      const cbs = callbacks; // these can be reset by callbacks
      ...
      for (let i = 0; i < cbs.length; ++i) cbs[i](instance);
    ...
    for (let i = 0; i < initHooks.length; ++i) initHooks[i](instance);
    return instance;
  ...
  var commands = (CodeMirror.commands = {
    ...
    goLineStartSmart: function (cm) {
      ...
      const firstNonWS = Math.max(0, text.search(/\S/));
      cm.setCursor(
        cur.line,
  ...
  function lookupKey(name, extraMap, map, handle, stop) {
    function lookup(map) {
      ...
      const fallthrough = map.fallthrough;
      if (fallthrough == null) return false;
      if (Object.prototype.toString.call(fallthrough) != "[object Array]") {
        return lookup(fallthrough);
      }
      for (let i = 0, e = fallthrough.length; i < e; ++i) {
  ...
  function isModifierKey(event) {
    const name = keyNames[e_prop(event, "keyCode")];
  ...
  function updateMarkedSpans(oldFirst, oldLast, startCh, endCh, newText) {
    ...
    let first = markedSpansBefore(oldFirst, startCh);
    const last = markedSpansAfter(oldLast, endCh);

    // Next, merge those two ends
    const sameLine = newText.length == 1;
    const offset = lst(newText).length + (sameLine ? startCh : 0);
    if (first) {
    ...
    const newMarkers = [newHL(newText[0], first)];
    if (!sameLine) {
      // Fill gap with whole-line-spans
      const gap = newText.length - 2;
      let gapMarkers;
      if (gap > 0 && first) {
        for (var i = 0; i < first.length; ++i) {
          if (first[i].to == null) {
  ...
  function newHL(text, spans) {
    return spans ? { text, markedSpans: spans } : text;
  }
  ...
  function detachMarkedSpans(line) {
    ...
    if (!spans) return;
    for (let i = 0; i < spans.length; ++i) {
      const lines = spans[i].marker.lines;
      const ix = indexOf(lines, line);
      lines.splice(ix, 1);
    }
  ...
  function attachMarkedSpans(line, spans) {
    if (!spans) return;
    for (let i = 0; i < spans.length; ++i) {
      const marker = spans[i].marker.lines.push(line);
    }
  ...
  Line.prototype = {
    ...
    highlight: function (mode, state, tabSize) {
      const stream = new StringStream(this.text, tabSize);
      const st = this.styles || (this.styles = []);
      let pos = (st.length = 0);
      if (this.text == "" && mode.blankLine) mode.blankLine(state);
      while (!stream.eol()) {
        const style = mode.token(stream, state);
        const substr = stream.current();
    ...
    getTokenAt: function (mode, state, tabSize, ch) {
      const txt = this.text;
      const stream = new StringStream(txt, tabSize);
    ...
    getContent: function (tabSize, wrapAt, compensateForWrapping) {
      ...
      function span_(html, text, style) {
        ...
        if (!specials.test(text)) {
          ...
          while (true) {
            ...
            pos += skipped + 1;
            if (m[0] == "\t") {
              const tabWidth = tabSize - (col % tabSize);
      ...
      if (wrapAt != null) {
        let outPos = 0;
        const anchor = (pre.anchor = elt("span"));
        span = function (html, text, style) {
          const l = text.length;
          if (wrapAt >= outPos && wrapAt < outPos + l) {
            const cut = wrapAt - outPos;
            if (cut) {
              span_(html, text.slice(0, cut), style);
              // See comment at the definition of spanAffectsWrapping
              if (compensateForWrapping) {
                const view = text.slice(cut - 1, cut + 1);
                if (spanAffectsWrapping.test(view)) {
      ...
      if (!allText && wrapAt == null) {
        ...
        var style;
        const sg = 0;
        let nextChange = marked[0].from || 0;
        const marks = [];
        let markpos = 0;
        const advanceMarks = function () {
          let m;
          while (
          ...
          nextChange =
            markpos < marked.length ? marked[markpos].from : Infinity;
  ...
  LeafChunk.prototype = {
    ...
    remove: function (at, n, callbacks) {
      for (let i = at, e = at + n; i < e; ++i) {
        ...
        line.cleanUp();
        if (line.handlers) {
          for (let j = 0; j < line.handlers.length; ++j) {
    ...
    insertHeight: function (at, lines, height) {
      ...
      for (let i = 0, e = lines.length; i < e; ++i) lines[i].parent = this;
    },
  ...
  function BranchChunk(children) {
    ...
    let size = 0;
    let height = 0;
    for (let i = 0, e = children.length; i < e; ++i) {
      const ch = children[i];
      size += ch.chunkSize();
      height += ch.height;
      ch.parent = this;
  ...
  BranchChunk.prototype = {
    ...
    insert: function (at, lines) {
      let height = 0;
      for (let i = 0, e = lines.length; i < e; ++i) height += lines[i].height;
        ...
        if (at <= sz) {
          ...
          if (child.lines && child.lines.length > 50) {
            while (child.lines.length > 50) {
              const spilled = child.lines.splice(child.lines.length - 25, 25);
              const newleaf = new LeafChunk(spilled);
  ...
  function lineNo(line) {
    ...
    for (let chunk = cur.parent; chunk; cur = chunk, chunk = chunk.parent) {
      for (let i = 0, e = chunk.children.length; ; ++i) {
  ...
  History.prototype = {
    addChange: function (start, added, old) {
      ...
      if (cur && !this.closed && this.compound) {
        ...
        this.done.push([{ start, added, old }]);
        this.closed = false;
      } else {
        const startBefore = Math.max(0, last.start - start);
        const endAfter = Math.max(
          0,
          start + old.length - (last.start + last.added),
        );
  ...
  function e_prop(e, prop) {
    const overridden =
      e.override && Object.prototype.hasOwnProperty.call(e.override, prop);
  ...
  function connect(node, type, handler, disconnect) {
    if (typeof node.addEventListener === "function") {
      ...
      const wrapHandler = function (event) {
  ...
  const lineSep = (function () {
    const te = elt("textarea");
    te.value = "foo\nbar";
