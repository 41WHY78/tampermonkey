...
(function () {
  ...
  function pushInBuffer(str) {
    buf += str;
  }
  function pushRepeatCountDigit(digit) {
    return function (cm) {
  ...
  function iterTimes(func) {
    for (let i = 0, c = getCountOrOne(); i < c; ++i) func(i, i == c - 1);
    clearCount();
  }
  ...
  function iterObj(o, f) {
    for (const prop in o) if (o.hasOwnProperty(prop)) f(prop, o[prop]);
  }
  ...
  function toCombo(ch) {
    // t -> T, T -> Shift-T, * -> '*', " " -> "Space"
    if (ch == " ") return "Space";
    const specialIdx = SPECIAL_SYMBOLS.indexOf(ch);
      ...
      for (; pos != stop; pos += dir) {
        for (let i = 0; i < regexps.length; ++i) {
          if (regexps[i].test(line.charAt(pos))) {
            ...
            for (
              ;
              pos != stop && regexps[i].test(line.charAt(pos));
              pos += dir
            ) {}
  ...
  function goLineStartText(cm) {
    ...
    const cur = cm.getCursor();
    const firstNonWS = cm.getLine(cur.line).search(/\S/);
  ...
  function charIdxInLine(cm, cHar, motion_options) {
    ...
    const line = cm.getLine(cur.line);
    let idx;
    const ch = toLetter(cHar);
    const mo = motion_options;
    if (mo.forward) {
  ...
  function moveTillChar(cm, cHar, motion_options) {
    ...
    const idx = charIdxInLine(cm, cHar, motion_options);
    const cur = cm.getCursor();
  ...
  function showAlert(cm, text) {
    const esc = text.replace(/[<&]/, function (ch) {
      return ch == "<" ? "&lt;" : "&amp;";
    });
  ...
  // main keymap
  const map = (CodeMirror.keyMap.vim = {
    ...
    "Shift-D": function (cm) {
      ...
      let lineN = cursor.line;
      const line = cm.getLine(lineN);
    ...
    "/": function (cm) {
      const f = CodeMirror.commands.find;
      f?.(cm);
      sdir = "f";
    },
    "'?'": function (cm) {
    ...
    N: function (cm) {
      const fn = CodeMirror.commands.findNext;
    ...
    "Shift-G": function (cm) {
      repeatCount == 0
        ...
        : cm.setCursor(repeatCount - 1);
      clearCount();
      CodeMirror.commands.goLineStart(cm);
    },
    "':'": function (cm) {
      const exModeDialog = ': <input type="text" style="width: 90%"/>';
      dialog(cm, exModeDialog, ":", function (command) {
        let c = false;
        if (command.match(/^\d+$/)) {
          cm.setCursor(command - 1, cm.getCursor().ch);
        } else {
          if (command.match(/^w[q!]?/)) {
            cm.execCommand("save", command.search("!") != -1);
            c = true;
          }
          if (command.match(/^[a-z]?q[!]?/)) {
            cm.execCommand("close", command.search("!") != -1);
            c = true;
          }
        }
        if (!c) {
  ...
  iterObj(
    {
      ...
      "'~'": function (cm) {
        const cur = cm.getCursor();
        let cHar = cm.getRange(
          { line: cur.line, ch: cur.ch },
          { line: cur.line, ch: cur.ch + 1 },
        );
        cHar =
          cHar != cHar.toLowerCase() ? cHar.toLowerCase() : cHar.toUpperCase();
        cm.replaceRange(
          cHar,
          { line: cur.line, ch: cur.ch },
          { line: cur.line, ch: cur.ch + 1 },
  ...
  CodeMirror.keyMap["vim-prefix-g"] = {
    E: countTimes(function (cm) {
      cm.setCursor(moveToWord(cm, word, -1, 1, "end"));
    }),
    "Shift-E": countTimes(function (cm) {
      cm.setCursor(moveToWord(cm, bigWord, -1, 1, "end"));
    }),
    G: function (cm) {
  ...
  CodeMirror.keyMap["vim-prefix-d"] = {
    ...
    B: function (cm) {
      const cur = cm.getCursor();
      const line = cm.getLine(cur.line);
      const index = line.lastIndexOf(" ", cur.ch);
  ...
  function findMatchedSymbol(cm, cur, symb) {
    let line = cur.line;
    var symb = symb || cm.getLine(line)[cur.ch];

    // Are we at the opening or closing char
    const forwards = ["(", "[", "{"].indexOf(symb) != -1;

    const reverseSymb = (function (sym) {
      switch (sym) {
        case "(":
          return ")";
        case "[":
    ...
    let disBal = forwards ? 0 : 1;

    while (true) {
      if (line == cur.line) {
        // First pass, do some special stuff
        var currLine = forwards
          ? cm.getLine(line).substr(cur.ch).split("")
          : cm.getLine(line).substr(0, cur.ch).split("").reverse();
      } else {
        var currLine = forwards
          ? cm.getLine(line).split("")
      ...
      for (let index = 0; index < currLine.length; index++) {
        ...
        if (disBal == 0) {
          if (forwards && cur.line == line) return { line, ch: index + cur.ch };
          else if (forwards) return { line, ch: index };
          else return { line, ch: currLine.length - index - 1 };
        }
      }
  ...
  function selectCompanionObject(cm, revSymb, inclusive) {
    ...
    const end = findMatchedSymbol(cm, cur, revSymb);
    const start = findMatchedSymbol(cm, end);
  ...
  function findBeginningAndEnd(cm, symb, inclusive) {
    ...
    if (chars[cur.ch] == symb && end == null) {
      start = cur.ch + 1;
    } // assign start to ahead of the cursor
    else {
  ...
  function offsetCursor(cm, line, ch) {
    const cur = cm.getCursor();
    return { line: cur.line + line, ch: cur.ch + ch };
  }
  ...
  const motions = {
    J: function (cm, times) {
      return offsetCursor(cm, times, 0);
    },
    Down: function (cm, times) {
      return offsetCursor(cm, times, 0);
    },
    K: function (cm, times) {
      return offsetCursor(cm, -times, 0);
    },
    Up: function (cm, times) {
      return offsetCursor(cm, -times, 0);
    },
    L: function (cm, times) {
      return offsetCursor(cm, 0, times);
    },
    Right: function (cm, times) {
      return offsetCursor(cm, 0, times);
    },
    Space: function (cm, times) {
      return offsetCursor(cm, 0, times);
    },
    H: function (cm, times) {
      return offsetCursor(cm, 0, -times);
    },
    Left: function (cm, times) {
      return offsetCursor(cm, 0, -times);
    },
    Backspace: function (cm, times) {
      return offsetCursor(cm, 0, -times);
    },
    B: function (cm, times, yank) {
      return moveToWord(cm, word, -1, times, "start", yank);
    },
    "Shift-B": function (cm, times, yank) {
      return moveToWord(cm, bigWord, -1, times, "start", yank);
    },
    E: function (cm, times, yank) {
      return moveToWord(cm, word, 1, times, "end", yank);
    },
    "Shift-E": function (cm, times, yank) {
      return moveToWord(cm, bigWord, 1, times, "end", yank);
    },
    W: function (cm, times, yank) {
      return moveToWord(cm, word, 1, times, "start", yank);
    },
    "Shift-W": function (cm, times, yank) {
      return moveToWord(cm, bigWord, 1, times, "start", yank);
    },
    "'^'": function (cm, times) {
      const cur = cm.getCursor();
      const line = cm.getLine(cur.line).split("");
    ...
    "'$'": function (cm) {
      const cur = cm.getCursor();
      const ch = cm.getLine(cur.line).length;
      return { line: cur.line, ch };
    },
    "'%'": function (cm) {
      return findMatchedSymbol(cm, cm.getCursor());
    },
    Esc: function (cm) {
      cm.setOption("keyMap", "vim");
      repeatCount = 0;
  ...
  iterObj(motions, function (key, motion) {
    CodeMirror.keyMap["vim-prefix-d"][key] = function (cm) {
      ...
      const end = motion(cm, repeatCount || 1, true);
    ...
    CodeMirror.keyMap.vim[key] = function (cm) {
      const cur = motion(cm, repeatCount || 1);
  ...
  function regexLastIndexOf(string, pattern, startIndex) {
    for (let i = startIndex == null ? string.length : startIndex; i >= 0; --i) {
  ...
  const textObjectList = ["W", "Shift-[", "Shift-9", "[", "'", "Shift-'"];
  const textObjects = {
    W: function (cm, inclusive) {
      const cur = cm.getCursor();
      const line = cm.getLine(cur.line);

      const line_to_char = new String(line.substring(0, cur.ch));
      const start = regexLastIndexOf(line_to_char, /[^a-zA-Z0-9]/) + 1;
      const end = motions.E(cm, 1);

      end.ch += inclusive ? 1 : 0;
      return { start: { line: cur.line, ch: start }, end };
    },
    "Shift-[": function (cm, inclusive) {
      return selectCompanionObject(cm, "}", inclusive);
    },
    "Shift-9": function (cm, inclusive) {
      return selectCompanionObject(cm, ")", inclusive);
    },
  ...
  function textObjectManipulation(cm, object, remove, insert, inclusive) {
    ...
    const tmp = textObjects[object](cm, inclusive);
    const start = tmp.start;
    const end = tmp.end;
  ...
  for (var i = 0; i < textObjectList.length; ++i) {
    const object = textObjectList[i];
    (function (object) {
      CodeMirror.keyMap["vim-prefix-di"][object] = function (cm) {
        textObjectManipulation(cm, object, true, false, false);
      };
      CodeMirror.keyMap["vim-prefix-da"][object] = function (cm) {
        textObjectManipulation(cm, object, true, false, true);
      };
      CodeMirror.keyMap["vim-prefix-yi"][object] = function (cm) {
        textObjectManipulation(cm, object, false, false, false);
      };
      CodeMirror.keyMap["vim-prefix-ya"][object] = function (cm) {
        textObjectManipulation(cm, object, false, false, true);
      };
      CodeMirror.keyMap["vim-prefix-ci"][object] = function (cm) {
        textObjectManipulation(cm, object, true, true, false);
      };
      CodeMirror.keyMap["vim-prefix-ca"][object] = function (cm) {
        textObjectManipulation(cm, object, true, true, true);
      };
    })(object);
  }
})();
