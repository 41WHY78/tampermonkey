...
MirrorFrame.prototype = {
  ...
  replace: function () {
    ...
    // prompting replace.
    const from = prompt("Enter search string:", "");
    let to;
    if (from) to = prompt("What should it be replaced with?", "");
    if (to == null) return;

    const cursor = this.mirror.getSearchCursor(from, false);
  ...
  macro: function () {
    const name = prompt("Name your constructor:", "");
    if (name) {
  ...
  reindentall: function () {
    this.mirror.autoIndentRange(
      { line: 0 },
      { line: this.mirror.lineCount() - 1 },
    );
  },
};
