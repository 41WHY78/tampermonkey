...
if (
  ...
  (function () {
    ...
    var domContentLoaded = function () {
      if (V || EV || D)
        console.log("content: detected DOMContentLoaded " + Eventing.contextId);
      domLoaded = true;
      // help if domcontentloader listener is not installed at the page :-/
      if (allReady)
        _handler.sendMessage(
          "domContentLoaded = true; if (typeof runAllLoadListeners !== 'undefined') runAllLoadListeners();",
        );
    };
    ...
    var domNodeInserted = function (node) {
      if (!nodeInserted) {
        if (V || EV || D)
          console.log("content: first DOMNodeInserted " + Eventing.contextId);
        nodeInserted = true;
      }
    };
    ...
    var _handler = {
      ...
      sendMessage: function (msg) {
        ...
        if (this.initstate == 0) {
          ...
          is +=
            "var TMwin = { backup: {}, use: " + JSON.stringify(use) + " };\n";
          is += "var TMJSON = JSON;\n";
          ...
          is +=
            "        if (!JSON.parse || JSON.parse != 'function parse() { [native code] }') {\n";
          is +=
            "              if (TMJSON && TMJSON.parse == 'function parse() { [native code] }') {\n";
          is += "                  JSON = TMJSON;\n";
          ...
          is +=
            "var Eventing = " +
            Helper.serialize(Eventing).replace(
              "___eval___",
              "eval",
            ) /* do a little trick to allow YUI compression */ +
            ";\n";
          is += "Eventing.init();\n";
          ...
          is +=
            "if (ENV) console.log('page: env initialized (" +
            Eventing.contextId +
            ")');\n";
          ...
          if (!use.safeContext) {
            ...
            is +=
              "    var st = document.getElementById('TM_script_tag_" +
              Eventing.contextId +
              "');\n";
            is += "    if (st && st.parentNode) {\n";
            is +=
              "        if (ENV) console.log('page: script tag cleanup (" +
              Eventing.contextId +
              ")');\n";
          ...
          if (k == responseId) {
            ...
            try {
              ...
              if (!_handler.responses[k]) {
                console.log(
                  "Warn: content: responseId " + k + " is undefined!!!",
                );
              } else {
              ...
              console.log(
                "content: Json parse error (runResponse):" + e + "\n" + args,
              );
            }
    ...
    var tmCE = {
      ...
      onContentResponse: function (key, tabId, responseId, json) {
        // (String key, int tabId, long responseId, String json) {
        if (_background) {
          if (V)
            this.log(
              "onContentResponse " + this.id + " " + responseId + " " + json,
            );
      ...
      onResponse: function (key, tabId, responseId, json) {
        // (String key, int tabId, long responseId, String json) {
        if (_background) {
          try {
            if (V)
              this.log("onResponse " + this.id + " " + responseId + " " + json);
            var j = Converter.encode(json);
            var l =
              "if (TMwin.chromeEmu) TMwin.chromeEmu.runResponse(" +
              responseId +
              ', "' +
              j +
              '")';
      ...
      onConnectResponse: function (key, tabId, responseId, json) {
        // (String key, int tabId, long responseId, String json) {
        if (_background) {
          try {
            if (V)
              this.log(
                "onConnectResponse " + this.id + " " + responseId + " " + json,
              );
            var j = Converter.encode(json);
            var l =
              "if (TMwin.chromeEmu) TMwin.chromeEmu.runConnectResponse(" +
              responseId +
              ', "' +
              j +
              '")';
      ...
      onContentRequest: function (request, sender, responseId) {
        if (_background) {
          ...
          var j = Converter.encode(
            JSON.stringify({ sender: sender, request: request }),
          );
          var l =
            "if (TMwin.chromeEmu) TMwin.chromeEmu.runContentRequest(" +
            responseId +
            ', "' +
            j +
            '", 0);';
      ...
      onMessage: function (request, sender, responseId) {
        if (_background) {
          ...
          var l =
            "if (TMwin.chromeEmu) TMwin.chromeEmu.runRequest(" +
            responseId +
            ', "' +
            j +
            '", 0)';
      ...
      xmlHttpRequest: function (key, details, responseId) {
        if (_background) {
          ...
          var load = function (r) {
            ...
            r.response = null;
            tmCE.onConnectResponse(
              key,
              0,
              responseId,
              JSON.stringify({ onLoad: true, response: r, r_id: id }),
            );
          };
          var readystatechange = function (r) {
            tmCE.onConnectResponse(
              ...
              0,
              responseId,
              JSON.stringify({ onReadyStateChange: true, response: r }),
            );
          };
          var error = function (r) {
            tmCE.onConnectResponse(
              ...
              0,
              responseId,
              JSON.stringify({ onError: true, response: r }),
            );
          };
          var done = function (r) {
            ...
            tmCE.onConnectResponse(
              ...
              0,
              responseId,
              JSON.stringify({ onDone: true, onDisconnect: true, response: r }),
            );
          };
      ...
      runUpdateListener: function () {
        // (int tabId, String details, String url) {
      ...
      sendExtensionMessage: function (key, json, responseId) {
        //(String key, String json, long responseId) {
        if (_background) {
          if (V)
            this.log(
              "sendExtensionMessage " + this.id + " " + responseId + " " + json,
            );
      ...
      sendExtensionConnect: function (key, json, responseId) {
        //(String key, String json, long responseId) {
        if (_background) {
          var obj = JSON.parse(json);
          obj.responseId = responseId;
          ...
          if (V)
            this.log(
              "sendExtensionConnect " + this.id + " " + responseId + " " + json,
            );
          var oMresponse = function (resp) {
            tmCE.onConnectResponse(
              ...
              0,
              responseId,
              JSON.stringify({ onMessage: true, msg: resp }),
            );
          };
          var oDresponse = function (resp) {
            tmCE.onConnectResponse(
              ...
              0,
              responseId,
              JSON.stringify({ onDisconnect: true }),
            );
            obj = null;
          };
      ...
      sendExtensionPortMessage: function (key, json, responseId) {
        //(String key, String json, long responseId) {
        if (_background) {
          if (V)
            this.log(
              "sendExtensionPortMessage " +
                this.id +
                " " +
                responseId +
                " " +
          ...
          if (!port) {
            this.log(
              "Error: sendExtensionPortMessage unable to find port " +
                responseId,
            );
          } else {
            var obj = JSON.parse(json);
      ...
      sendTabsRequest: function () {
        // (String key, int tabId, String json, long responseId) {
        console.log("WARN: not supported!");
      },
      ...
      updateTab: function () {
        //(String key, String tabId, String json) {
        console.log("WARN: not supported!");
      },
      ...
      storageRemoveItem: function () {
        //(String key, String vkey) {
        console.log("WARN: not supported!");
      },

      storageSetItem: function () {
        //(String key, String vkey, String value) {
        console.log("WARN: not supported!");
      },
    ...
    var initUnsafe = function () {
      if (use.safeContext) {
        ...
        is += "};\n";
        is +=
          "document.addEventListener('TM_do" +
          Eventing.contextId +
          "', eventHandler, false);\n";
        f += is;
            ...
            if (!get[k].forceUnsafe) {
              try {
                if (obj) {
                  ...
                  if (D)
                    console.log(
                      "env: define " + k + " to " + JSON.stringify(obj),
                    );
                  Clean.push(function () {
                    if (CV) console.log("clean: prop window[" + k + "]");
                    prop[k].value = null;
      ...
      for (var o = 0; o < arr.length; o++) {
        var wrap = function () {
          ...
          if (!obj) {
            ...
            Clean.push(function () {
              if (CV)
                console.log("clean: " + Helper.toType(k) + "[wrappedJSObject]");
            ...
            if (event == eDOMATTRMODIFIED) {
              ...
              if (this.outerHTML) {
                ...
                try {
                  ...
                  if (!logged)
                    console.log(
                      "content: WARN unable to use MutationObserver -> fallback to DOMSubtreeModified event",
                    );
                  logged = true;
    ...
    Eventing = {
      ...
      init: function () {
        ...
        if (V)
          Eventing.log("Eventing initialized (" + Eventing.contextId + ")");
      ...
      fireEvent: function (data, evt) {
        ...
        if (ENV)
          Eventing.log("fireEvent " + evt + " -> " + JSON.stringify(data));
        try {
          var customEvent = document.createEvent("MutationEvent");
          customEvent.initMutationEvent(
            evt,
      ...
      registerListener: function () {
        if (ENV)
          Eventing.log(
            "registerListener " + Eventing.rEventId + Eventing.contextId,
          );
        if (_background) {
          Eventing.eventSource.addEventListener(
            Eventing.rEventId + Eventing.contextId,
      ...
      eventHandlerPage: function (evt) {
        try {
          ...
          try {
            ___eval___(
              j,
            ); /* do a little trick to allow YUI compression, window['eval'] resets the context :( */
            if (TS)
              Eventing.log(
                "it took " +
                  (new Date().getTime() - evt.timeStamp) +
                  " ms to process this event ->" +
                  j.fn,
              );
          } catch (e) {
            console.log(
              "page:Error: processing event (" + j + ")! " + e.message,
            );
          }
          j = "";
        } catch (e) {
      ...
      eventHandlerContent: function (evt) {
        try {
          ...
          try {
            ...
            Eventing.log(
              "Error: processing event (" + j.fn + ")! " + JSON.stringify(e),
            );
          }
          j = "";
        } catch (e) {
      ...
      cleanup: function () {
        if (Eventing.eventSource) {
          Eventing.eventSource.removeEventListener(
            Eventing.rEventId + Eventing.contextId,
            Eventing.eventHandler,
            false,
          );
      ...
      run: function () {
        if (!allReady && env && emu) {
          ...
          if (domLoaded) {
            ...
            if (V || EV || D)
              console.log(
                "content: Start ENV with DOMContentLoaded " +
                  Eventing.contextId,
              );
            ...
            if (V || EV || D)
              console.log(
                "content: Start ENV with loadHappened " + Eventing.contextId,
              );
          }
          if (load != "" && (V || EV)) {
    ...
    function reqListerner(request, sender, sendResponse) {
      if (!allReady) {
        window.setTimeout(function () {
          reqListerner(request, sender, sendResponse);
        }, 10);
        return true;
      }
      if (wannaRun) {
    ...
    var forceTestXhr = function () {
      ...
      if (D)
        console.log(
          "content: create test XHR to check whether webRequest API is working",
        );
      var d = {
        method: "GET",
      ...
      var done = function () {
        var req = { method: "getWebRequestInfo", id: Eventing.contextId };
    ...
    var init = function () {
      ...
      var updateResponse = function (resp) {
        if (resp === undefined) {
          if (D)
            console.log(
              "content: _early_ execution, connection to bg failed -> retry!",
            );
        ...
        if (resp.enabledScriptsCount) {
          if (V || D)
            console.log(
              "content: start event processing for " +
                Eventing.contextId +
                " (" +
                resp.enabledScriptsCount +
                " to run)",
            );
          wannaRun = true;
          ...
          if (V || D)
            console.log(
              "content: disable event processing for " + Eventing.contextId,
            );
          wannaRun = false;
          allReady = true;
          secondStage.cleanupHlp();
